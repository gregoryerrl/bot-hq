...
| `docs/stream-json-events.md` | empirical claude-code stream schema (frozen since 2026-05; the raw captures it was written from were removed 2026-09-09 — they were real sessions, and no test ever loaded them) | — |
...
| `src/core/git.rs` | `hardened(repo)`: the `git -C <repo> -c core.fsmonitor=false -c core.hooksPath=<nonexistent>` every automatic invocation in a registered repo uses (Apply-tab diff/untracked listing, worktree ops, stale-refs corpus, start-sha, hook install) + `DIFF_HARDENING_ARGS` (`--no-ext-diff --no-textconv`) — a delivered repo's local config must not run programs on open | S |
...
| `src/signaling/jsonrpc.rs` | `dispatch` (initialize/ping/tools/list/tools/call) + `call_tool` (one flat match over 41 tools), capability gate, refusal rows, `PARITY_HOLD` (empty) | XL |
...
| `src/signaling/bridge/cl_write.rs` | `cl_write_file` + `cl_edit_file` over one guarded `write_cl` path (`WriteOp`): path guards, atomic write, shrink guard, retired-term diff, `git_version_library` (add -A + commit), push trigger, `sweep_project` | L |
...
| `src/appimage_env.rs` | strips payload-rooted entries ($APPDIR) from the env of everything bot-hq SPAWNS, under an AppImage launch only: structural rule (split on `:`, drop entries under the payload, remove the var if nothing survives) rather than a name blocklist, `PATH` never removed, no-op emitted when nothing was dropped. Exists because a Linux AppImage exports LD_LIBRARY_PATH/PYTHONHOME/PATH/GTK_* into every child, which hard-broke `git`-over-HTTPS, `curl`, `python3`, `flatpak` and Spectacle on Fedora 2026-08-25 — and made `gsettings` report the payload's GNOME defaults instead of the host's. Scrubbing in `AppRun` instead would strip the payload libs from WebKitWebProcess and re-break the EGL no-window bug | S |
...
| `src/tauri_cmd/files.rs` | `read_workspace_file` (path-guarded, size-capped): `allowed_roots` = repo (`Hidden::SecretClass` — `.git`, `.env*`, key/credentials files refused, `.github`/`.gitignore` viewable) + temp dirs + library + the session's claude-code `tool-results` dirs (`claude_tool_results_dirs`, all `Hidden::AllDotted`); deepest containing root wins | M |
...
| `frontend/src/hooks/useDragResize.ts`, `frontend/src/hooks/useFocusTrap.ts`, `frontend/src/hooks/useEscapeKey.ts`, `frontend/src/hooks/useListEditor.ts`, `frontend/src/hooks/useServerDraft.ts`, `frontend/src/hooks/useNow.ts`, `frontend/src/hooks/usePointerSwap.ts` | small hooks (`useNow`: a clock for deadlines nothing announces — the needs-you view's temporary-halt wake; `usePointerSwap`: the dashboard's drag-to-swap on pointer events, because the Tauri webview never fires an HTML5 drop) | S |
...
| `frontend/src/lib/theme.ts` | the `color-scheme` `index.css` declares to the browser — `theme.test.ts` reads the REAL `index.css` and pins `:root { color-scheme: dark }`. Native controls (`<select>` popups, scrollbars, spinners) are drawn by the platform, so without it they follow the host theme: on Fedora/KDE the AppImage's GTK hook forced `Adwaita:light` and the phase `<select>` rendered as a light pill in a dark header | S |
