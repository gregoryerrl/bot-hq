...
> external driver MCP server (removed 2026-08-17), the eval harness `bench/`
> (removed 2026-08-17), the `forward_events` table
...
## 2026-08-25 (Fedora smoke test) — five Linux defects, four fixed

First run of bot-hq on a Fedora 44 / KDE Wayland box, from a fresh clone, with the
app itself running as the repaired AppImage payload
(`scripts/install-appimage-linux.sh`, `c6cb74f6`). The app works — this session
ran inside it — but smoking it end-to-end surfaced five defects, four of them in
bot-hq's own code and none of them visible on macOS. Full diagnosis in
`docs/FEDORA-LINUX-COMPAT.md`.

**The environment leak (new `src/appimage_env.rs`).** An AppImage exports a
payload-rooted environment (`LD_LIBRARY_PATH`, `PATH`, `PYTHONHOME`, `PERLLIB`,
`QT_PLUGIN_PATH`, the `GTK_*`/`GIO_*`/`GSETTINGS_*` family) and **every child
bot-hq spawned inherited it** — nothing in `src/` had ever called `env_clear` or
`env_remove`. Host binaries then loaded the payload's Ubuntu-22.04 libraries:
`git` over HTTPS, `curl`, `python3`, `flatpak` and Spectacle all hard-failed in
agent shells and in the user's own Terminal tab (`ldd /lib64/libcurl.so.4`
resolves twelve deps into the payload). Worse than failing, `gsettings` *lied*: loading the
payload's glib/gio left it with no dconf backend (the payload's gio module dir
ships only `libgiognutls.so`), so every key returned its schema default and a
theme probe read `'default'` for a desktop that is `'prefer-dark'`. The
intuitive explanation — schema-dir shadowing — is wrong and was believed here
long enough to nearly produce a false correction against a correct diagnosis. Fixed by a structural scrub — drop `$APPDIR`-rooted entries
from each `:`-separated value, remove the var if nothing survives — never a name
blocklist, since `AppRun` is generated. `PATH` is never removed (falls back to
`/usr/bin:/bin`), and when nothing is dropped **no op is emitted at all**, so
values the rule was never meant to touch stay byte-identical (six live vars ship
a trailing `:`; `DEBUGINFOD_URLS` is space-separated with embedded colons).
Guarded on `APPDIR` set AND the exe living under it, so source builds are a
strict no-op — and deliberately spawn-site-only, because scrubbing in `AppRun`
would strip the payload libs from `WebKitWebProcess` and re-break the EGL
no-window bug `c6cb74f6` just fixed. Applied at `agents/spawn.rs`,
`core/terminal.rs`, `policy/tool_gate.rs` and `signaling/bridge/cl_push.rs` (that
one pushes over the network and died on the same libcurl symbol).

**Native controls rendered light on a dark desktop** — the user's report. The
AppImage's GTK hook probes GNOME's `gtk-theme` key, reads `'Breeze'` on KDE,
finds no "dark" substring and forces `GTK_THEME=Adwaita:light`; WebKitGTK draws
`<select>` popups from the GTK theme, so the phase select rendered as a light
pill in a dark header (screenshot captured). Fixed with `:root { color-scheme:
dark; }` + `frontend/src/lib/theme.ts`/`theme.test.ts` rather than by patching
the hook (and verified after the fact by rendering both variants in the host's
WebKitGTK 2.52.5 under `GTK_THEME=Adwaita:light`: without the declaration the
`<select>` is a light pill, with it the control is dark — so it overrides the
GTK theme rather than merely agreeing with it) — the hook is regenerated by linuxdeploy every build and is not in this
repo, and the same latent bug exists on any light-mode Windows or macOS host.

**`webview_screenshot` was macOS-only, shipped to three platforms in 1.0.0.**
`/usr/sbin/screencapture` hardcoded with no `cfg` gate, called unconditionally,
failing at runtime off macOS — and telling Linux and Windows users to open a
macOS Settings pane. Now gated: Linux picks Spectacle/`grim`/`import`/
`gnome-screenshot`, Windows says so plainly.

**Node 26 fails the frontend suite** (43 tests, one root cause). Node ships a
native `localStorage` global that stays `undefined` without
`--localstorage-file`; vitest will not overwrite an existing global and aliases
`window` to `globalThis`, so there is no jsdom Storage left to borrow. Shimmed in
`test-setup.ts` rather than pinned — `release.yml`'s `node-version: 22` is what
the bundle is BUILT with and has never run a test, since **no CI job runs the
tests at all**. `engines` + `.nvmrc` added so "Node 22+" stops being prose with
nothing behind it.

**Not fixed, deliberately:** Tauri's `APPDIR` security WARN on every extracted-
payload launch (cosmetic; the alternative is faking an `APPIMAGE` path), and the
packaging follow-ups from the original investigation (stop bundling
`libwayland-*`, ship an RPM) — the AppImage build is still not reproducible from
this host.

Gates at close: vitest **543/543** on Node 26 AND on Node 22 in a container —
the two runs exercise opposite branches of the shim (on 22 the global does not
exist, so vitest installs jsdom's own `Storage` and the shim declines;
`constructor.name` is `Storage` there, `Object` on 26). Baseline before the fix
was 494/537. `tsc --noEmit` clean. One load-dependent flake seen under parallel
container load (`RolesPanel.test.tsx`), green in isolation and on both full
runs. The Rust
side is unit-verified in isolation (13/13 for the scrub module, compiled verbatim
against real `tokio`/`portable-pty`) but **`cargo test` on the full tree has not
run** — the Tauri build dependencies need a `sudo dnf install` that was still
pending with the user at close. Named as outstanding rather than implied green.
