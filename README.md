...
`approve_finding` needs `approve_finding`; `cl_write_file`, `cl_edit_file` and `cl_register_folder_description`
need `write_context_library`; `close_session` needs `close_session` (D16). The seeded pair — HANDS holds the
...
The trust boundary — what bot-hq trusts, what it does not, and how to report a
