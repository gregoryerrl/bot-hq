...
got fixed (dropdowns, etc)" — the person who reported the bug observing it gone
in the real app, which shows there is no regression in practice.

That confirmation is corroboration, not the proof, and the order matters: the
instance he observed runs with `GTK_THEME` **unset**, so the host KDE theme
(`prefer-dark`) applies and a dark `<select>` could be explained by the host
theme alone. The forced-`Adwaita:light` A/B above is what isolates
`color-scheme` as the cause, because it produces a dark control while the theme
is light. Proof from the A/B; independent confirmation from the real app. It is the portable lever: the
...
The repaired payload's WEB PROCESS still aborts on this host after the EGL fix.
It is a different failure from both of the ones above, and it is not fixable
from this repository.

**Scope, stated before the evidence, because these are easy to conflate:** what
follows explains a `WebKitWebProcess` abort captured by `coredumpctl`. It does
NOT explain the app process exiting. Today's log records **22 `bot-hq starting`
lines and only 2 clean shutdowns**, so most terminations logged nothing at all,
and `journalctl -k` shows no OOM kill or segfault in the same window. Whether
