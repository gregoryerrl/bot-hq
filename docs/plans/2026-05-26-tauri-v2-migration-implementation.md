...
**Branch:** `tauri-v2-migration` in worktree `~/Projects/bot-hq-tauri`. Push grant: session-level, scope=specific, branches=["tauri-v2-migration"]. Commit conventions follow the repo's resolved policy (enforced by the git hooks).
