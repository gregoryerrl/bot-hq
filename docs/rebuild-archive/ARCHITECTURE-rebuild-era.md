...
One code path serves the upstream API vendor, DeepSeek, and any vendor-compatible API.
...
- **claude-code:** the upstream CLI tool that wraps a language model. We spawn one subprocess per agent via `-p` mode.
