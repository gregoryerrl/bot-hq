...
- A.2 `[P]` Distill **`general-rules.md`** from current `rulebook.md`, `glossary.md`, `roles.md`, `mcp-tool-manifest.md`, `agent-onboarding.md`. Universal rules only (security, the `ask_user_choice`/`mark_awaiting_user` MCP conventions). Slim and tight.
...
- A.4 `[P]` For each relevant project, distill **`projects/<p>/conventions.md`** (patterns, glossary, anti-patterns) + **`projects/<p>/notes.md`** (learnings worth carrying forward). Initial targets: `bot-hq` (self-referential, light) and two client projects.
