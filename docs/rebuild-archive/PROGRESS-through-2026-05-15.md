...
4. **Per-project rules migration from old CL.** The autonomous build's Phase A distilled high-level conventions/notes per project but missed the operational rules in `~/.bot-hq/projects/<project>.yaml`. Migrated:
   - `client-a` + `client-a-exporter` (two client engagements): added **Gates (strict)** sections (push approval, force-push token format `force-push-greenlight: {branch}@{sha}`, coder/sub-agent tool blocklist, per-action commit approval) and **Read-only DB access** notes. Augmented the existing client-facing-hygiene sections with the scaffold-scan rule (and the commit-footer anti-pattern for the exporter).
   - `client-b`: expanded the single-line Gates summary into the full strict-default policy.
   - `client-c` and `bot-hq`: no changes — autonomous build's distillation already covered everything material; the legacy `project_feedback` entries for bot-hq self-project (snap_footer, arc_closure, greenflag_authority, etc.) were intentionally skipped as they encode old hub-coordination patterns that don't apply to the new architecture.
...
- [x] A.4 Per-project conventions + notes (bot-hq plus four client projects — 9 files total)
