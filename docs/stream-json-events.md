...
Empirical capture from `claude-code 2.1.141` on macOS. The raw captures this
was written from (`plain.jsonl`, `tool_use.jsonl`, `malformed.jsonl`) were
removed from the repository on 2026-09-09: they were real sessions, and the
schema below is what they established. Re-run the probes whenever the CLI
version changes — schema is informal and may shift.
