...
describe("HaltBanner", () => {
  ...
  it("puts an agent's close card in the gate slot (0081)", () => {
    // Approve closes the session, Reject keeps it open — a gate that latches
    // the ring while the user decides, so it renders where gates render and
    // never counts as a tray question.
    expect(isApproval({ kind: "close", options: ["Approve", "Reject"] })).toBe(true);
    expect(isTrayItem({ kind: "close", options: ["Approve", "Reject"] })).toBe(false);
  });
