...
- HALT-ALL-WORK (H-31, H-33): on receiving FLAG whose content contains EITHER the substring "agent <id> at <N>%, halt" (Emma context-cap fire at ≥95% per-pane context usage) OR the substring "plan usage at <N>%, halt" (Emma plan-cap fire at ≥95% account-scoped 5h/7d window utilization), finish current tool call, post final SNAP via hub_session_close, post a brief handoff message to user, then idle. User restarts trio in a fresh-context session; H-15 ledger pre-loads context. Do NOT attempt to continue work in the current session — the halt exists because at least one agent is past auto-compact runway OR the account is at the plan-window cap. Match by substring meaning across BOTH triggers (agent context-cap OR plan-usage), not regex anchors. Both fire HALT-ALL-WORK.
