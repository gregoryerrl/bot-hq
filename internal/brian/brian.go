...
- HALT-ALL-WORK (H-31): on receiving FLAG whose content contains the substring "agent <id> at <N>%, halt" (Emma context-cap fire at ≥95% usage on any agent), finish current tool call, post final SNAP via hub_session_close, post a brief handoff message to user, then idle. User restarts trio in a fresh-context session; H-15 ledger pre-loads context. Do NOT attempt to continue work in the current session — the halt exists because at least one agent is past auto-compact runway. Match by substring meaning, not regex anchors.
