...
// TestIPAVFirstTask_endToEnd is the T-2.6 IPAV-first-task validation per
// phase-t.md v5: at least one full IPAV cycle (I→P→Implement→V) executed
...
//   - T-2.1 IPAVRuntime (OpenTask + TransitionPhase + RecordPhaseUsage + CompleteTask)
...
//   - T-1.7 IPAV-state-tracking schema in CL (CL.SaveIPAVState/IPAVState)
//
...
func TestIPAVFirstTask_endToEnd(t *testing.T) {
	cl := newTestCL(t)
	...
	// Persist fault-tree alongside IPAV state
	...
	t.Logf("=== T-2.6 IPAV-FIRST-TASK VALIDATION PASS ===")
	...
	t.Logf("Wired components VERIFIED: IPAVRuntime + faulttree + hypothesis + cl_strong_style_assign + R44 + R47 + IPAV-state-tracking-in-CL")
}
