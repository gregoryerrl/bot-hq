...
func newTestRuntime(t *testing.T) *IPIVRuntime {
	t.Helper()
	cl := newTestCL(t)
...
func TestNewIPIVRuntime_validation(t *testing.T) {
	cl := newTestCL(t)

	if _, err := NewIPIVRuntime(nil, "bot-hq"); err == nil {
		t.Error("expected error for nil CL")
	}
	if _, err := NewIPIVRuntime(cl, ""); err == nil {
