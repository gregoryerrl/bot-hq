...
// fires the context-cap halt-flag. 95% leaves a small runway for the duo to
...
// see internal/emma/plan_usage.go for the matching producer.
...
func (g *SystemMonitor) checkContextCap(now time.Time) {
	...
	for _, s := range snap {
		...
		// Phase-S-followup: context-cap critical emit goes through
		// daemoncron unconditionally. SetHaltActive side-effect stays
