...
func TestRunHealthChecksRoutesToRain(t *testing.T) {
	...
	if !strings.Contains(src, want) {
		t.Errorf("emma.go must contain %q — anomaly reports route to Rain (EYES)", want)
	}
	if strings.Contains(src, `ToAgent:   "brian",`) {
		t.Errorf("emma.go must not route anomalies to Brian — that violates EYES/HANDS split")
	}
}
...
func TestAgentImbalanceExcludesCoders(t *testing.T) {
	agents := []protocol.Agent{
		...
		{ID: "emma", Type: protocol.AgentEmma, Status: protocol.StatusOnline},
