...
// NewWakeOnlyForTest builds a minimal Emma whose only wired dependency is
...
func NewWakeOnlyForTest(db *hub.DB, stop chan struct{}) *Emma {
	return &Emma{db: db, stopCh: stop}
}
...
func (g *Emma) RunWakeDispatchLoopForTest() { g.wakeDispatchLoop() }
