...
// the absolute window cap so the duo can finish the current tool call
...
func (g *SystemMonitor) checkPlanUsage(now time.Time) {
	...
	// halt-system behavior; weekly quota decay is opaque-to-duo.
...
// from hub.db halt_state at Emma startup. Closes the asymmetry between
