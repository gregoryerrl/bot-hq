...
// in DB (e.g., persisted from prior session), but THIS Emma instance
...
func TestPlanCapHaltActiveSeededFromDBOnRestart(t *testing.T) {
	...
	// Sanity: in-mem bool starts at zero-value false (fresh Emma instance).
	if g.planCapHaltActive {
		t.Fatal("test setup invalid: planCapHaltActive should start false on fresh Emma instance")
	}
