...
func TestWakeScheduleEndToEnd(t *testing.T) {
	...
	// emma.New()/Start() because that boots Ollama and the full agent
	...
	defer close(stop)
	g := emma.NewWakeOnlyForTest(db, stop)
	...
	for time.Now().Before(deadline) {
		...
		for i := range msgs {
			if msgs[i].FromAgent == "system" && msgs[i].Content == "wake-up: re-test 3" {
				dispatched = &msgs[i]
				break
			}
		}
...
func TestWakeCancelBeforeFireBlocksDispatch(t *testing.T) {
	...
	for _, m := range msgs {
		if m.FromAgent == "system" && m.Content == "should-not-fire" {
