...
// Z-3: bot_hq_agent_bootstrap MCP tool DELETED. Daemon-paste replaces it:
// brian/rain spawnTmux invokes contextload.RenderSessionBootstrap and
// pastes the result into the agent's initial prompt at spawn-time. Per
// architecture/sessions-as-containers.md "Bootstrap moves to daemon"
// section — bootstrap is something the daemon DOES to the agent, not
// something the agent performs. bot_hq_context_load stays for ad-hoc
// cross-project reads (different lifecycle).
