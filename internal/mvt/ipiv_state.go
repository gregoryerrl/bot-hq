...
// Stage represents the current IPIV pipeline stage.
type Stage string
...
// DefaultPhaseBudget returns the default IPIV budget per phase-t.md v5.
...
// ValidatePhaseTransition checks if a phase-transition is permitted per IPIV.
...
func TaskStatePath(homeDir, projectID, taskID string) string {
	return filepath.Join(homeDir, ".bot-hq", "projects", projectID, "tasks", taskID, "ipiv-state.yaml")
}
