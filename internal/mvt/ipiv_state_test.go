...
func TestSaveLoad_roundTrip(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "ipiv-state.yaml")
...
func TestTaskStatePath_canonical(t *testing.T) {
	...
	want := "/home/user/.bot-hq/projects/proj-1/tasks/task-abc/ipiv-state.yaml"
	if got != want {
...
func TestEndToEndCycle_simulated(t *testing.T) {
	// Simulated end-to-end IPIV cycle on a representative task per phase-t.md v5
	// T-0.5 MVT done-criteria: "MVT prototype demonstrates IPIV cycle on real task end-to-end".
	dir := t.TempDir()
	path := filepath.Join(dir, "ipiv-state.yaml")
	models := map[string]string{"brian": "claude-default", "rain": "deepseek-v4-pro"}
	...
	t.Logf("E2E IPIV cycle SIMULATED PASS: 4 phase-transitions / total-cost $%.2f / final %s", totalCost, loaded.VerifyResult)
}
