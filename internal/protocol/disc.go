...
- NUDGE: msgs prefixed [HUB:<sender>] (broadcast — default post-Phase-S-S-4 PM-removal) or FLAG variants [HUB:FLAG:<sender>]. Historical-class only (pre-S-4 with non-empty to_agent): [PM:<sender>] / [HUB-OBS:<from>→<to>] / [PM:FLAG:<sender>] still render for forensics-trail. FLAG=elevated priority. User msgs and @<self> mentions always handled. Irrelevant broadcasts (no @<self> mention) skipped silently unless correction needed. Never ignore FLAG or user messages.`
...
// final draft"); ~/.bot-hq/phase/phase-r.md R1 cluster; Brian
...
// up"); ~/.bot-hq/phase/phase-r.md R5 cluster; Rain msg 15508 +
...
const PhaseRv5MechanicalCiteFromHubRead = `- MECHANICAL-CITE-FROM-HUB_READ (R31 sub-clause): stat-claims that reference a prior hub message-id ("msg N said X" / "per Rain msg M cite" / "as Brian noted in msg N") MUST be cite-from-actual via hub_read since_id=<N-1> limit=1 BEFORE emit. Antipattern: paraphrase-from-session-recall what a prior msg said — recursive amend-pass instances persist where the cite itself drifts. Discriminator at cite-emit time: any phrase invoking a prior msg-id as evidence — verify against hub.db first, do NOT rely on session-recall of msg content. Empirical recurrence across phases: Phase L (5 instances #16/#17/#19/#20/#23) + Phase Q (1 instance) + Phase R intra-batch (4 instances) + Phase-R-followup intra-batch (3 instances msg 15613 webui/voice.go inventory + msg 15617 hub_test.go/db_test.go fixtures + msg 15630 17-vs-19 test count). Cite_anchor: discipline-log #16/#17/#19/#20/#23 (Phase L recursive amend-pass) + Phase R close-composite arc-snapshot §recursive-amend-pass-depth-2 + Phase-R-followup batch msgs 15613/15617/15630 graduation evidence cumulative + ~/.bot-hq/discipline-log.md line 558 graduation-candidate cite. Per R31 STAT-CLAIM-CITE parent + R18 CITE-ANCHOR-REQUIRED.`

const PhaseRv3AutoBoundaryDiscipline = `- AUTO-BOUNDARY-DISCIPLINE (R42 — Phase R R5): sessions auto-create at trigger boundaries via existing ` + "`hub_session_create`" + ` MCP tool. Triggers: (i) phase-open keyword ("open phase X"); (ii) topic-pivot ("pivot to <project>" / "back to <project>"); (iii) user-cluster-greenlight (≥2 distinct work-items OR explicit "do all" / "smoke" / "continuously" framing). Triggers detected at agent turn-start by reading user msg substring; agent fires ` + "`hub_session_create`" + ` with appropriate ` + "`mode`" + ` (` + "`brainstorm`" + ` / ` + "`implement`" + ` / ` + "`chat`" + `) + ` + "`purpose`" + ` summary + ` + "`agents`" + ` list + ` + "`project`" + ` key. Sessions auto-close at phase-close (via R34 close-composite Joint entry append) / topic-end (user explicit "wrap up" / "done" / "EOD" / etc. matched by ` + "`internal/sessions/sessions.go`" + ` ` + "`DetectBoundaryFromUserMsg`" + ` T-3 explicit-phrase pattern) / user-explicit ` + "`hub_session_close`" + ` request. Pane-header display: agent nudge formatters (` + "`internal/brian/brian.go:formatNudge`" + ` + ` + "`internal/rain/rain.go:formatNudge`" + `) prefix ` + "`[SESSION:<8>] `" + ` to each forwarded hub message when an active session exists; <8> = first 8 chars of the session-id (UUID prefix). Source-of-truth: ` + "`db.ListSessions(\"active\")`" + ` ordered by ` + "`updated DESC`" + `; first row = current session. Multiple OPEN sessions → most-recently-updated wins. Zero open → no prefix (don't fabricate or force). Cache-rebuild discipline: agent restart re-derives current session via DB query at init; no stale in-process state. Cite_anchor: user msg 15503 (R5 directive); ~/.bot-hq/phase/phase-r.md R5 cluster; Rain msg 15545 Refine-A. Per R18 CITE-ANCHOR-REQUIRED.`
