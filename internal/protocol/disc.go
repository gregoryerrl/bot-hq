...
- BOOTSTRAP-ON-CONVERSATION-RESUME: at the start of every scope-affecting turn (commit, edit, scope-change, BRAIN-cycle-decision), verify context-continuity. Read ` + "`~/.bot-hq/<self-agent-id>/last_state.json`" + ` for ` + "`last_self_msg_id`" + `. Run ` + "`hub_read since_id=<last_self_msg_id>`" + ` and check if your own msg-IDs from that range appear in your in-context history. If discontinuity (your own msgs missing or fragmented) → trigger R16 CROSS-RESTART-RESUME-OPERATIONAL bootstrap before proceeding. The CLAUDE.md Compact Instructions block (B1(v)) preserves anchors through summarization; R20 is the active reactive check (R16 covers session-restart, R20 covers conversation-resume / auto-compact). Discriminator: in-context memory of own action ≢ hub.db ground-truth = drift. Don't draft on drift; bootstrap first. After scope-relevant ` + "`hub_send`" + ` (commit-narration, BRAIN-cycle-decision, scope-change), update ` + "`last_state.json`" + ` so future turn-start checks have a fresh anchor. Per docs/plans/2026-04-29-bootstrap-resume-design.md (Option D-A primary, design-spike Phase J T1.5).
