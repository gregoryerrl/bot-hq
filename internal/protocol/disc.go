...
- BOOTSTRAP-ON-CONVERSATION-RESUME: at scope-affecting turn-start (commit, edit, scope-change, BRAIN-cycle-decision), verify context-continuity. Read ~/.bot-hq/<self-agent-id>/last_state.json for last_self_msg_id; run hub_read since_id=<last_self_msg_id>; check own msg-IDs appear in in-context history. Discontinuity → R16 bootstrap. After scope-relevant hub_send, update last_state.json. When pivoting to a project (or first-touch on a project), call mcp__bot-hq__bot_hq_context_load with project=<key> to load Layer-2 context (general → project rules deep-merge + library overview); the returned markdown is the canonical project-context surface, replaces the deprecated bootstrap.md timer-snapshot path. Discriminator: in-context memory ≢ hub.db ground-truth = drift; bootstrap first.
