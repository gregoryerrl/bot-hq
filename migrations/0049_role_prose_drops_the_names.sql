-- ============================================================================
-- 0049 — the role prose stops naming agents.
--
-- rc3 **D10** (docs/plans/2026-08-11-rc3-decisions.md). The user, repeatedly:
-- *"I already said multiple times that I'm dropping the names, only the Role +
-- Model Name."* A participant is displayed as its ROLE and its MODEL, and the
-- agents' own prompts opened with `You are **Brian**` / `You are **Rain**`.
--
-- The prose is user-editable data since 0046, so changing the Rust constants is
-- only half the change: an install that has already run 0046/0048 serves the
-- OLD prose out of its database forever (migrations never re-run), while a
-- fresh install serves the new text — and the suite stays green through the
-- whole divergence. `storage::participants::tests::seeded_role_prose_is_byte_
-- identical_to_the_hardcoded_constants` is the oracle that relates the two, and
-- this file is the other half it compares against.
--
-- WHAT CHANGED IN THE TEXT, exactly:
--   * both roles' H1 and opening line — `# Role — Brian (HANDS)` /
--     `You are **Brian**. You are HANDS in the BRAIN duo. Your peer is Rain
--     (EYES, review-only). Together you are BRAIN.` become `# Role — HANDS` /
--     `You are **HANDS**. Who else is in this session, and what each of them may
--     do, is listed at the end of this prompt.` The roster sentence is DELETED
--     rather than reworded because layer 2 already generates it from the live
--     roster (rc3 D4) — `agents::capability_prompt::render` emits
--     `## Participants in this session` naming every enabled peer and what it
--     holds. Two participants of the same role, a renamed role, a third
--     participant: the generated line is right in all of them and a hand-written
--     one was right in none.
--   * every other occurrence of `Brian` becomes `HANDS` and every `Rain` becomes
--     `EYES` — they are ROLE names, which the user owns and can rename. The
--     possessives follow (`Brian's` → `HANDS'`), and the pronouns with them
--     (`his`/`he`/`her` → `its`/`it`/`their`), because a role is not a person.
--   * `the duo` becomes `the session` wherever it meant "the running pair", and
--     the `EYES-sign-off gate` heading becomes `Review sign-off gate`.
--   * HANDS' finding-gate paragraph stops naming who files: "A participant that
--     holds the finding capability can file BLOCKING findings" (rc3 D11 — the
--     product must not encode what a role MEANS).
--
-- No rule, tool name, or refusal was added or removed. The deny-list block, the
-- Bash enumeration, the observations-only rule, the bottom-up review order and
-- the CL opener are byte-identical apart from the substitutions above; the
-- guards in `agents::prompts::tests` are what hold that.
--
-- Byte counts: HANDS 10205 -> 10275, EYES 13354 -> 13389.
--
-- THE TEXT BELOW IS VERBATIM `HANDS_ROLE` / `EYES_ROLE`, dumped by rustc from
-- the resolved literals (`cargo run --example dump_role_prose`), as 0046 and
-- 0048 were. Hand-transcribing them is how the two silently diverge.
--
-- WHY THE `description_prompt = '<the previous seed>'` GUARD rather than an
...
-- edited row alone. `IS NULL` is included because 0046 is unapplied on a fresh
-- database only in the sense that it runs first — a row it skipped would still
-- be NULL here.
--
-- The database is being reset (docs/plans/2026-08-12-db-reset-runbook.md), so
-- there is no history to migrate: `messages.author = 'brian' | 'rain'` rows are
-- legacy data and are deliberately NOT backfilled (D10). This file is structure
-- and seed only.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1. HANDS — verbatim `HANDS_ROLE`, overwriting 0046's seed only.
-- ---------------------------------------------------------------------------
...
'
WHERE slug = 'hands'
  AND (description_prompt IS NULL OR description_prompt = '# Role — Brian (HANDS)
...
-- ---------------------------------------------------------------------------
-- 2. EYES — verbatim `EYES_ROLE`, overwriting 0048's re-seed only.
-- ---------------------------------------------------------------------------
