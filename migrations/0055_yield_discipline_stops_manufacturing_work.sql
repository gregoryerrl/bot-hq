-- ============================================================================
-- 0055 — the yield discipline stops manufacturing work (rc3 D35).
--
-- Found live in `s-86a81478`: HANDS, queue empty, said — in chat — "I won't
-- halt on a state they haven't acted on" and invented a CL-delta review chore
-- instead; three participants volleyed over it for 25 minutes while the user
-- sat unable to type. The prose it was obeying was written for the old world,
-- where a halt was a tray row and re-halts spammed the bell. Under D35 a halt
-- is the session's ONE slot (re-declaring replaces, harmlessly) and stopping
-- is what gives the user the floor — so "never yield twice; work anything
-- workable" taught exactly the wrong move.
--
-- WHAT CHANGED IN THE TEXT: two paragraphs of the opening block.
--   1. The `ask_user_choice` paragraph: a parked question no longer halts the
--      session (D35), so "the session stays halted until it does" became "the
--      session keeps working; the pick arrives batched with the user's next
--      message", and "after you call it, STOP" became "carry on with whatever
--      else is workable — or pass".
--   2. The `mark_awaiting_user` paragraph: a halt STOPS the session and the
--      reason is the recap the user reads; declare once then pass; re-declaring
--      is harmless but adds nothing; and — the s-86a81478 rule — "Do not
--      invent work to avoid stopping."
-- Nothing else moved. EYES' prose is untouched (13501 bytes, unchanged).
--
-- Byte counts: HANDS 10569 -> 10741.
--
-- THE TEXT BELOW IS VERBATIM `HANDS_ROLE`, dumped by rustc from the resolved
-- literal (`cargo run --example dump_role_prose`), as 0046-0050 were.
-- Hand-transcribing it is how the two silently diverge.
--
-- The `description_prompt = '<0050''s seed>'` guard, as always: the column is
-- the USER's to edit, and a migration must never overwrite their prose. 0050 is
-- APPLIED and immutable; this file is the sanctioned way to move the text, and
-- `seeded_role_prose_is_byte_identical_to_the_hardcoded_constants` is the
-- oracle that catches divergence.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1. HANDS — verbatim `HANDS_ROLE`, overwriting 0050's seed only.
-- ---------------------------------------------------------------------------
