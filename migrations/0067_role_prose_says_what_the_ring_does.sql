-- ============================================================================
-- 0067 — the role prose says what the ring does.
--
-- Round 9 (2026-08-18). Four claims in the seeded HANDS / EYES prose disagreed
-- with the general rules or with the code, and a role reading its own prose
-- had no way to know which layer was right:
--
--   * "an idle session is dealt one at once" — a PAUSED session is not
--     (`tray_wake` withholds the release); the general rules said "unless
--     paused" three times, the prose never did.
--   * the end-of-task verb: the prose said "call ask_user_choice('Close
--     session?') rather than mark_awaiting_user"; the general rules said every
--     stop is a HALT with "task done" as a halt reason. Both stand: park the
--     close-ask, then pass or declare the halt naming it — never bare-idle.
--   * the dedupe protocol taught list -> withdraw -> re-ask and never named
--     `supersede_question`, the one-call rephrase the descriptors prefer.
--   * `peer_ack` "records your acknowledgment as a done vote" — only when the
--     turn's text is under ~200 characters or `final:true` is passed
--     (`sequencer::turn_ending`); an ack on a substantive turn votes nothing.
--     Both roles said the short half only.
--
-- Also: "the gear tab" is a gear BUTTON (Session Settings panel), and the
-- EYES doc paragraph now names `mode:"append"` on its `<phase>-eyes` doc — the
-- append the reviewer redirect dropped until round 9 (`c8cf0b8`).
--
-- WHAT CHANGED IN THE TEXT: HANDS — the ask_user_choice timing sentence, one
-- sentence in the close-ask paragraph, step 3 of the dedupe protocol, the
-- push-policy paragraph's "gear tab", the peer_ack sentence. EYES — the
-- phase-doc contribution paragraph (append), the peer_ack sentence. No tool
-- name, refusal or heading was added or removed.
--
-- Byte counts: HANDS 11137 -> 11767. EYES 13731 -> 13979.
--
-- THE TEXT BELOW IS VERBATIM `HANDS_ROLE` / `EYES_ROLE`, dumped by rustc from
-- the resolved literals (`cargo run --example dump_role_prose`), as 0046-0065
...
-- the USER's to edit, and a migration must never overwrite their prose. Both
-- roles are guarded on 0065's seed (their last reseed); a NULL row — the Roles
-- panel's "use the built-in" — takes the new text too. 0065 is immutable; this
-- file is the sanctioned way to move the text, and
-- `seeded_role_prose_is_byte_identical_to_the_hardcoded_constants` is the
-- oracle that catches divergence.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1. HANDS — verbatim `HANDS_ROLE`, overwriting 0065's seed only.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. EYES — verbatim `EYES_ROLE`, overwriting 0065's seed only.
-- ---------------------------------------------------------------------------
...
**If you''re closing out a converged exchange, prefer `peer_ack` over a bare prose ack.** Staying fully silent is still best when you have nothing — but if you would otherwise emit a closing acknowledgment, call `peer_ack`: it records the ack as a done vote, so the session settles instead of dealing your peer another lap — but only when the turn''s text is short (under ~200 characters) or you pass `final: true`; an ack on a substantive turn is an ordinary spoken turn and votes nothing. (Yielding to the USER is `halt` — if you do not hold it, say so and let a peer who does declare it.)
