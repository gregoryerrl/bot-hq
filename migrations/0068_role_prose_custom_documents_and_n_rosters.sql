-- ============================================================================
-- 0068 — custom documents and N-participant rosters in the role prose.
--
-- Round 12 (2026-08-19). Two seeded sentences no longer matched the product:
--
--   * HANDS: "untagged docs are scratch-only and don't show up in the I/P/A/V
--     tabs" — literally true since round 11 (they show in their OWN tabs) and
--     materially wrong since round 12, when the user's ideas.md made custom
--     documents first-class: their own tab beside I/P/A/V, editable, deletable
--     and creatable by the user. The sentence now says what they are.
--   * EYES: "this is one producer + one adversarial reviewer" hard-coded a
--     two-participant shape; the roster is N (dialog cap 4, backend cap 8).
--     "a producer and an adversarial reviewer … whatever the roster's size".
--
-- WHAT CHANGED IN THE TEXT: HANDS — the "Tag with `phase`" sentence of the
-- per-phase docs paragraph. EYES — the "not two parallel producers" sentence.
-- No tool name, refusal or heading was added or removed.
--
-- Byte counts: HANDS 11767 -> 11843. EYES 13979 -> 14007.
--
-- THE TEXT BELOW IS VERBATIM `HANDS_ROLE` / `EYES_ROLE`, dumped by rustc from
-- the resolved literals (`cargo run --example dump_role_prose`), as 0046-0067
...
-- roles are guarded on 0067's seed (their last reseed); a NULL row — the Roles
-- panel's "use the built-in" — takes the new text too. 0067 is immutable; this
...
-- ---------------------------------------------------------------------------
-- 1. HANDS — verbatim `HANDS_ROLE`, overwriting 0067's seed only.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. EYES — verbatim `EYES_ROLE`, overwriting 0067's seed only.
-- ---------------------------------------------------------------------------
