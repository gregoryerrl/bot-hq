-- 1.0.0 Batch 6 (M7c): the role-prose reseed that renames `eyes_flag` to
-- `flag_finding` in the PRESET prose (the tool keeps `eyes_flag` as an
-- accepted alias, so an UNMOVED row still works). Standard guarded reseed
-- (0049-family): the UPDATE fires only on a row still carrying the previous
-- seed byte-for-byte — a user-edited row is never touched (their prose keeps
-- the alias name, which the dispatch accepts forever). Text dumped by rustc
-- via `cargo run --example dump_role_prose` before and after the constant
-- edit, never hand-transcribed. On a FRESH DB (0072 deleted the seeded pair)
-- both UPDATEs match 0 rows, which is a success; preset installs copy the
-- updated constants directly.
...
'
 WHERE slug = 'hands' AND description_prompt = '# Role — HANDS
...
- Still explain the finding in chat too — `flag_finding` is the enforcement; chat is the conversation. And you don''t have to win the argument with HANDS: a rebuttal you disagree with surfaces to the user, who adjudicates. Flag honestly; let the gate + the user hold the line.
...
'
 WHERE slug = 'eyes' AND description_prompt = '# Role — EYES
