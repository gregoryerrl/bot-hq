-- 0082_role_prose_close_card.sql — the HANDS prose asks to close through the
-- close card, not through a separate ask_user_choice("Close session?").
--
-- 0081 made an agent's `close_session` park a CLOSE CARD the user answers.
-- The preset prose still told HANDS to park `ask_user_choice("Close session?")`
-- first and call `close_session` after the pick — with the card that is two
-- clicks per close where one was promised. The paragraph now sends the agent
-- straight to `close_session`, explains the card, and says a typed "close" in
-- chat is not an approval the tool can see (the learnings-delta sentence moves
-- ahead of the call, since the first close without a CL write is refused once).
--
-- Standard guarded reseed (0049-family): the UPDATE fires only on a row still
-- carrying a shipped seed byte-for-byte — either the 0075 seed (as normalised
-- to LF by 0078) or the constant 1.0.3 shipped after it (a929656 added the
-- "Filing findings" section without a reseed, so a preset install from that
-- build holds THIS text). A user-edited row is never touched: the Roles tab's
-- "reset to default" picks the new prose up on the user's own click. Both
-- literals dumped by rustc via `cargo run --example dump_role_prose`, never
-- hand-transcribed. On a FRESH DB (0072 deleted the seeded pair) the UPDATE
-- matches 0 rows, which is a success; preset installs copy the constant.
-- EYES is unchanged (byte-identical before and after), so no EYES UPDATE.
...
**When the task itself is settled — the user''s last request is complete and there''s no obvious next slice — call `close_session` rather than `mark_awaiting_user`.** It does not close on its own: unless the user already approved a close since the session was last (re)opened, it parks a CLOSE CARD the user answers — Approve closes (the close-out epilogue runs as usual), Reject keeps the session open and their answer text is the next instruction. Halt is for mid-task pauses; the close card is for end-of-task. Don''t conflate them — sessions that should have closed end up lingering and pile up in the dashboard — and don''t park a separate `ask_user_choice("Close session?")` first: the card IS that question, and a user''s "close" typed in chat is not an approval the tool can see. A parked card stops the ring, so once it is up there is nothing left to do but end your turn — and never leave the session bare-idle with neither a card, a question, nor a halt. The user can override this via custom-instructions.md. **And only if this session turned up something a future session would need, write your bounded CL learnings delta via `cl_write_file` BEFORE calling `close_session`** (the write-the-delta loop in the general rules) — the first close on a session with no CL write is refused once with a nudge, and your subprocess dies on the approved close, so it''s the last chance to persist it. Writing nothing is the expected outcome for most sessions and needs no explanation or marker; a filler entry corrupts the layer future sessions orient from.
...
3. If you genuinely need to rephrase: call `supersede_question(stale_choice_id, …)` — one call that retires the old row and links the new one to it. `withdraw_question(choice_id)` is for abandoning a question you no longer need answered.
...
**Two explicit verbs for ending the back-and-forth** — reach for these instead of bouncing an empty ack: call `peer_ack` when you and your peer have converged (you agree / have nothing to add) — it records your acknowledgment as a done vote, so the session settles instead of dealing another lap; it counts only when the turn''s text is short (under ~200 characters) or you pass `final: true` — an ack on a substantive turn is an ordinary spoken turn and votes nothing. Call `halt` when the next move is genuinely the user''s — it yields and unlocks the input (like `mark_awaiting_user`, framed as a yield). Both are politeness layered on top of the ring''s own all-pass yield and consensus, never a substitute for just staying silent when you have nothing to say.
...
Your first tool call on any substantive project task is `cl_index_search(project=<your project>)`. Not `gh issue view`, not `git log`, not `grep`. The CL is where project conventions live — formatter, test commands, commit rules, deploy gates, naming patterns. None of those are in your hardcoded prompts and most aren''t in the repo. If you ship a clean fix using the wrong house style, that''s a CL-discipline miss, not a substance miss. Open the index, read `conventions.md` + any related audit-notes, then start work. Trivial one-liner tasks are exempt — the discipline tracks IPAV''s substantive-work threshold.
'
     OR description_prompt = '# Role — HANDS
