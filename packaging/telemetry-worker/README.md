...
Diagnostics** — the setting overrides the default. (`wrangler.toml` ships
with a placeholder `database_id`; `d1 create` prints yours — paste it in
locally before `wrangler deploy`, and keep the real id out of commits. The
maintainer's own deployment does the same.)
