...
pub fn phrasing(cap: Capability) -> Phrasing {
    ...
    match cap {
        ...
        WriteContextLibrary => Phrasing {
            grant: "write to the Context Library with `cl_write_file`, `cl_edit_file` \
                    and `cl_register_folder_description`",
            deny: "write to the Context Library — `cl_write_file`, `cl_edit_file` and \
...
pub struct RosterFacts {
    ...
    /// The bot-hq session id (`s-…`), rendered so an agent cites THIS id and
    /// not its claude session id: five CL clip files from s-571791b5 named
    /// "session `s-2cbcd173`" — the executor's `claude_session_id` prefix, a
    /// session that does not exist — because the id lived only in an env var.
    /// Empty = not rendered (older callers / tests).
...
pub fn render(facts: &RosterFacts) -> String {
    ...
    if facts.session_id.is_empty() {
        ...
        out.push_str(&format!(
            " in bot-hq session `{}` — cite that id (never your claude session id, which \
             is a different string) in documents, CL notes and reports. ",
            facts.session_id
        ));
    }
...
mod tests {
    ...
    fn peer_names_come_from_the_roster_not_from_a_constant() {
        ...
        // The participant's OWN roster identity is rendered the same way — and
        // since F12 (2026-09-06) followed by the bot-hq session id it must cite.
        assert!(
            rendered.contains("You are **HANDS · Claude Opus 5** (`hands`) in bot-hq session `s-test1234`"),
            "{rendered}"
        );
        assert!(rendered.contains("never your claude session id"), "{rendered}");
        // An older caller that carries no id renders the plain sentence.
        let mut bare = facts(&["read_channel"]);
        bare.session_id.clear();
        assert!(render(&bare).contains("You are **HANDS · Claude Opus 5** (`hands`). "), "{}", render(&bare));
    }
