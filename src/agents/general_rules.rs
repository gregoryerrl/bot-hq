...
What each user-facing tool COSTS (know this before choosing one): a **question** (`ask_user_choice`) parks in the user's tray and stops NOTHING — the session keeps working and the answer arrives as a user row (an idle session — unless paused — is dealt a turn for it at once; a working one reads it at its next boundary). An **approval** and a **halt** stop the whole session until the user acts. The archive study still holds: ~60% of questions asked were answerable by the agent — ask only what you cannot decide.
