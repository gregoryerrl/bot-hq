...
//! hand-written into [`RAIN_ROLE`] and are now generated by
...
/// The agent slug `Storage::ensure_session_roster` seeds for a role slug, or
/// `""` when it seeds none.
///
/// Two SQL literals own this mapping — `role_id` resolves through
/// `(SELECT id FROM roles WHERE slug = 'hands')` onto participant `'brian'`, and
/// `'eyes'` onto `'rain'`. Restated here rather than read from the database
/// because the question this answers is asked about a role that may not be in
/// any session yet. `builtin_prose_for_role_matches_what_the_seeded_roster
/// _falls_back_to` walks a real roster and fails if the two ever disagree.
fn seeded_agent_for_role(role_slug: &str) -> &'static str {
    match role_slug {
        "hands" => "brian",
        "eyes" => "rain",
        _ => "",
...
///
/// **This is the honest predicate for "clearing this box restores the default",
/// and the `builtin` column is not.** `read_system_prompt` treats an empty or
/// all-whitespace row as absent and falls back to [`role_for`], keyed on the
/// participant's AGENT slug. So whether a role has a default to restore is a
/// property of whether this file carries a constant reachable from that role —
/// nothing to do with who seeded the row.
///
...
/// with no instruction, when in fact it restores `BRIAN_ROLE` in full.
...
mod tests {
    ...
    fn rain_explicitly_forbids_mutating_tools() {
        ...
        assert!(RAIN_ROLE.contains("- **`Edit`, `Write`, `NotebookEdit`** — file writes."));
