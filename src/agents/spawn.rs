...
/// The model row's `cli_settings` (0079) as the map `--settings` is built from.
///
/// Empty when the row carries none, and — deliberately — empty with a warning
/// when it carries anything but a JSON object: claude-code silently ignores a
/// malformed `--settings` in `-p` mode, and on the edit-capable branch that
/// argument also carries the Tool-Gate hook, so one bad character in a model
/// row must never take the gate down with it. The Settings → Models save path
/// refuses non-objects too; this is the belt for a row written any other way.
fn model_cli_settings(
    config: &AgentConfig,
    agent_name: &str,
) -> serde_json::Map<String, serde_json::Value> {
    let Some(raw) = config.cli_settings.as_deref() else {
    ...
    match serde_json::from_str::<serde_json::Value>(raw) {
        ...
        Ok(_) => {
            warn!(
                ...
                "models.cli_settings is not a JSON object — ignored, nothing injected"
            );
            serde_json::Map::new()
        }
        Err(e) => {
            warn!(
                ...
                model = %config.model_name,
                error = %e,
                "models.cli_settings is not valid JSON — ignored, nothing injected"
            );
            serde_json::Map::new()
        }
    }
...
fn build_command(cfg: &SpawnConfig) -> Command {
    ...
    if !cfg.capabilities.grants(crate::agents::Capability::EditFiles) {
        ...
        // The model's own CLI settings (0079) ride `--settings` on THIS branch
        ...
        match std::env::current_exe() {
            Ok(exe) => {
                ...
                if let serde_json::Value::Object(ref mut map) = settings {
                    ...
                    // The model's CLI settings (0079) fill only the keys the
                    // role override left empty: a per-role override is the
                    // user's explicit word about THIS participant, a model row
                    // speaks for every participant on that model.
    ...
    if let Some(base) = crate::agents::llm_proxy::proxied_base_url(
...
mod tests {
    ...
    /// The `--settings` value of a built command, or `None` when the flag is
    /// absent — the read-only branch passed none at all until 0079.
    fn settings_arg_of(c: &SpawnConfig) -> Option<serde_json::Value> {
        let args = debug_command(c);
        let raw = args.iter().skip_while(|a| *a != "--settings").nth(1)?;
        Some(serde_json::from_str(raw).expect("--settings must be valid JSON"))
    }

    const FABLE_OVERRIDE: &str =
        r#"{"modelOverrides":{"claude-fable-5":"claude-fable-5-1"}}"#;

    /// 0079: a model row's `cli_settings` reaches the CLI on BOTH spawn
    /// postures. The read-only half is the wire that was missing: until this
    /// shipped, only the edit-capable branch built a `--settings` argument (for
    /// the Tool-Gate hook), so a `modelOverrides` fix for the executor left the
    /// reviewer on the same model's 200k default. Kill-test: delete the
    /// `model_cli_settings` block on either branch and its half goes red.
    #[test]
    fn model_cli_settings_reach_both_spawn_postures() {
        let mut hands = cfg();
        hands.config.cli_settings = Some(FABLE_OVERRIDE.into());
        let s = settings_arg_of(&hands).expect("edit-capable branch passes --settings");
        assert_eq!(
            s["modelOverrides"]["claude-fable-5"],
            serde_json::json!("claude-fable-5-1"),
            "got {s}"
        );
        assert!(
            s["hooks"]["PreToolUse"].is_array(),
            "the Tool-Gate hook must survive the merge: {s}"
        );

        let mut eyes = eyes_cfg();
        eyes.config.cli_settings = Some(FABLE_OVERRIDE.into());
        let s = settings_arg_of(&eyes).expect("read-only branch passes --settings too");
        assert_eq!(
            s["modelOverrides"]["claude-fable-5"],
            serde_json::json!("claude-fable-5-1"),
            "got {s}"
    ...
    /// A per-role override is the user's explicit word about THIS participant;
    /// the model row speaks for everyone on that model — so on a key both set,
    /// the override wins and the model fills only the gaps.
    #[test]
    fn a_role_override_outranks_the_model_row_on_a_shared_key() {
        use crate::claude_config::{save_overrides, ClaudeOverrides};
        let dir = tempfile::tempdir().unwrap();
        let mut store = ClaudeOverrides::default();
        store.per_role.entry("hands".into()).or_default().ultracode = Some(true);
        ...
        // The model row disagrees on `ultracode` AND adds a key of its own.
        c.config.cli_settings =
            Some(r#"{"ultracode":false,"modelOverrides":{"claude-fable-5":"claude-fable-5-1"}}"#.into());
        let s = settings_arg_of(&c).unwrap();
        assert_eq!(s["ultracode"], serde_json::json!(true), "override must win: {s}");
        assert_eq!(
            s["modelOverrides"]["claude-fable-5"],
            serde_json::json!("claude-fable-5-1"),
            "model row must fill the gap: {s}"
        );
    }

    /// A row that is not a JSON object injects nothing — and on the hook branch
    /// the hook is untouched. claude-code silently ignores a malformed
    /// `--settings` in `-p` mode, which would take the Tool Gate down with it.
    #[test]
    fn malformed_cli_settings_are_dropped_not_injected() {
        for bad in ["{not json", "[1,2]", "\"modelOverrides\"", "42"] {
            let mut hands = cfg();
            hands.config.cli_settings = Some(bad.into());
            let s = settings_arg_of(&hands).expect("hook branch always passes --settings");
            assert!(s["hooks"]["PreToolUse"].is_array(), "{bad}: hook lost: {s}");
            assert!(s.get("modelOverrides").is_none(), "{bad}: injected: {s}");
