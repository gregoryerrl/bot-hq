...
async fn spawn_session_handle(
        ...
        Ok(None) | Err(_) => {
            ...
            let captured: Option<String> = if let Some(repo) = working_repo_path.as_ref() {
                ...
                tokio::task::spawn_blocking(move || -> Option<String> {
                    let out = crate::core::git::hardened(&repo)
                        .args(["rev-parse", "HEAD"])
                        .output()
                        .ok()?;
    ...
    // Per slot, the window the registry claims for the model this participant
    // spawns on — the pump compares it with what the CLI reports (0079's
    ...
    for (slot, p) in live.iter().enumerate() {
        ...
        configured_windows.push(cfg.context_window.and_then(|w| u64::try_from(w).ok()));
...
///
/// `pub(crate)` because the agent-facing `cl_index_search` rows cap their
/// descriptions with it too: a byte-indexed slice panics on the em-dashes
/// those descriptions are full of, and a panic there kills every
/// participant's boot call.
...
fn boot_primer() -> &'static str {
    "[System: BOOT — orientation only, and this is not a turn.\n\n\
     ...
     note where things live. The boot is orientation, not study: check a row's \
     `bytes` before any whole-file Read, and pull the sections you need with \
     cl_retrieve instead. Nothing you write here is delivered to your peers, so \
...
pub(crate) async fn resolve_spawn_config(
    ...
    if let Ok(Some(mut cfg)) = storage.get_agent_config(agent_name).await {
        // `agent_configs` has no `cli_settings` column (0079 added it to the
        // registry only), so a row from this tier is structurally `None`. A
        // participant landing here — no model pick, or a role pointing at a
        // deleted registry row — on a model id the CLI does not know would run
        // at the 200k default: the bug 0079 exists to fix, reopened by the
        // fallback. Borrow the setting from the registry row that spawns the
        // same wire model, when there is one (EYES fd1802cd).
        if cfg.cli_settings.is_none() {
            if let Ok(Some(m)) = storage.model_by_name(&cfg.model_name).await {
                cfg.cli_settings = m.cli_settings;
            }
        }
        return cfg;
    }
...
mod tests {
    ...
    /// 1.0.5 (sweep S1): the signaling server refuses a session/agent pair
    /// with no secret registered, so the one spawn path must register a
    /// fresh secret for EVERY spawn — a `--resume` respawn after a relaunch
    /// included, since that is the client the old fail-open arm claimed to
    /// protect. The config built here for a participant that already holds a
    /// claude session id carries `--resume`, names a token URL, and that token
    /// is the one the bridge will accept.
    #[tokio::test]
    async fn a_resume_respawn_registers_a_fresh_secret_the_server_accepts() {
        let s = Storage::memory().await.unwrap();
        s.create_session("s1", "t", None).await.unwrap();
        s.ensure_session_roster("s1", crate::storage::MAX_SESSION_PARTICIPANTS).await.unwrap();
        let data_dir = TempDir::new().unwrap();
        ...
        let mcp_temp = TempDir::new().unwrap();
        let roster = s.participants_for_session("s1").await.unwrap();
        let mut me = roster.iter().find(|p| p.slug == "hands").expect("the executor").clone();
        me.claude_session_id = Some("claude-session-from-before-the-relaunch".into());
        let bridge = SignalingBridge::new();
        ...
        let argv = crate::agents::spawn::debug_command(&cfg);
        assert!(
            argv.windows(2).any(|w| w[0] == "--resume" && w[1] == "claude-session-from-before-the-relaunch"),
            "this is the respawn path: {argv:?}"
        );
        let mcp_json =
            std::fs::read_to_string(cfg.mcp_config_path.as_ref().expect("an mcp config")).unwrap();
        let url = mcp_json
            .split('"')
            .find(|s| s.starts_with("http://127.0.0.1:1/sessions/s1/hands/"))
            .expect("the config names the agent's MCP URL");
        let token = url
            .strip_prefix("http://127.0.0.1:1/sessions/s1/hands/")
            .and_then(|rest| rest.strip_suffix("/mcp"))
            .expect("the URL carries a token segment");
        assert!(!token.is_empty(), "token-less URL shape would be refused since 1.0.5");
        assert!(
            bridge.mcp_token_matches("s1", "hands", Some(token)),
            "the token written into the resumed agent's config must be the one registered"
        );
    ...
    const FABLE_CLI: &str = r#"{"modelOverrides":{"claude-fable-5":"claude-fable-5-1"}}"#;

    fn fable_row(id: &str) -> crate::storage::Model {
        crate::storage::Model {
            id: id.into(),
    ...
    /// row's `cli_settings` must reach the `AgentConfig` the spawn is built
    /// from. The `spawn.rs` tests set `config.cli_settings` by struct literal,
    /// so they pin only the injection half; this pins the resolution half.
    /// Kill-test: replace `cli_settings: m.cli_settings` with `None` on the
    /// explicit-model arm and this goes red while the whole suite stays green.
    #[tokio::test]
    async fn resolve_carries_cli_settings_from_the_chosen_model_row() {
        let s = Storage::memory().await.unwrap();
        s.upsert_model(&fable_row("m-fable")).await.unwrap();
        let resolved = resolve_spawn_config(&s, "hands", Some("m-fable")).await;
        assert_eq!(resolved.model_name, "claude-fable-5-1");
        assert_eq!(resolved.cli_settings.as_deref(), Some(FABLE_CLI));
        assert_eq!(resolved.context_window, Some(1_000_000));
    }

    /// Tier 3 (the registry default when there is no pick and no agent row):
    /// `default_spawn_model` prefers the named default generation, so the row
    /// carrying `cli_settings` here IS that default. Kill-test: null the
    /// `cli_settings: m.cli_settings` on the default arm.
    #[tokio::test]
    async fn resolve_carries_cli_settings_from_the_default_model_row() {
        let s = Storage::memory().await.unwrap();
        // The seeded registry has no `cli_settings`; give the named default one.
        let mut def = s
            .default_spawn_model()
            .await
            .unwrap()
            .expect("0073 seeds the default generation");
        assert_eq!(def.model_name, crate::storage::DEFAULT_SPAWN_MODEL_NAME);
        def.cli_settings = Some(FABLE_CLI.into());
        s.upsert_model(&def).await.unwrap();
        let resolved = resolve_spawn_config(&s, "nobody", None).await;
        assert_eq!(resolved.model_name, crate::storage::DEFAULT_SPAWN_MODEL_NAME);
        assert_eq!(resolved.cli_settings.as_deref(), Some(FABLE_CLI));
    }

    /// Tier 2 (EYES advisory fd1802cd): the `agent_configs` fallback has no
    /// `cli_settings` column, so it borrows the registry row that spawns the
    /// same wire model. Without this, a role pointing at a deleted registry row
    /// on an unknown model id ran at 200k — 0079's own bug, reopened by the
    /// fallback. Kill-test: delete the `model_by_name` borrow.
    #[tokio::test]
    async fn resolve_borrows_cli_settings_for_the_agent_config_tier_by_model_name() {
        let s = Storage::memory().await.unwrap();
        s.upsert_agent_config(&crate::storage::AgentConfig {
            agent_name: "hands".into(),
            provider: "anthropic".into(),
            model_name: "claude-fable-5-1".into(),
            base_url: None,
            auth_token: Some("agent-row-token".into()),
            updated_at: String::new(),
            context_window: None,
            cli_settings: None,
        })
        ...
        .unwrap();
        s.upsert_model(&fable_row("m-fable")).await.unwrap();
        // The pick names a registry row that no longer exists → tier 2.
        let resolved = resolve_spawn_config(&s, "hands", Some("m-deleted")).await;
        assert_eq!(resolved.auth_token.as_deref(), Some("agent-row-token"), "tier 2 reached");
        assert_eq!(resolved.cli_settings.as_deref(), Some(FABLE_CLI));
        // Control: a wire model no registry row spawns stays `None` — the tier
        // borrows, it never invents.
        s.upsert_agent_config(&crate::storage::AgentConfig {
            agent_name: "eyes".into(),
            provider: "anthropic".into(),
            model_name: "claude-nowhere".into(),
            base_url: None,
            auth_token: None,
            updated_at: String::new(),
            context_window: None,
        ...
        .await
        .unwrap();
        let none = resolve_spawn_config(&s, "eyes", None).await;
        assert_eq!(none.model_name, "claude-nowhere");
        assert_eq!(none.cli_settings, None);
    }
    ...
    ///
    /// F12 (week-35 dissection): the prompt names the bot-hq session id, in the
    /// participants section, with the warning that the claude session id is a
    /// different string — five CL clip files cited `s-2cbcd173` (a claude id
    /// prefix, no such session) because the id lived only in `BOT_HQ_SESSION_ID`.
    /// Kill-test: drop `session_id` from `resolve_roster_facts`' literal and
    /// this goes red.
    #[tokio::test]
    async fn the_prompt_names_the_bot_hq_session_id_in_the_roster_section() {
        let tmp = TempDir::new().unwrap();
        let paths = Paths::for_data_dir(tmp.path().to_path_buf());
        paths.init().unwrap();
        let s = Storage::memory().await.unwrap();
        s.create_session("s-abc12345", "t", None).await.unwrap();
        s.ensure_session_roster("s-abc12345", crate::storage::MAX_SESSION_PARTICIPANTS).await.unwrap();
        let roster = s.participants_for_session("s-abc12345").await.unwrap();
        let me = roster.iter().find(|p| p.slug == "hands").unwrap();
        let facts = resolve_roster_facts(&s, &roster, me).await.unwrap();
        assert_eq!(facts.session_id, "s-abc12345");
        let prompt = read_system_prompt(&paths, None, None, None, None, Some(&facts)).unwrap();
        let section = &prompt[prompt.rfind("## Participants in this session").unwrap()..];
        assert!(section.contains("in bot-hq session `s-abc12345`"), "got: {section}");
        assert!(section.contains("never your claude session id"), "got: {section}");
    }
