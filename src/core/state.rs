...
impl AppState {
    ...
    async fn teardown_session(&self, id: &str, archive: Option<bool>) -> Result<()> {
        ...
        // A QUEUED outward publish (0080) is named first: the sweep below
        // retires its row too, and silent loss is what the durable queue exists
        // to prevent.
        self.bridge.announce_queued_dropped_at_close(id).await;
