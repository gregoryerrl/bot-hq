            ...
            if state == SessionActivity::Idle {
                ...
                let pending_tray = if candidate {
                    match idle_watch.storage.has_pending_tray(&session_id).await {
                        // F9: an approved gate still EXECUTING is a wait too —
                        // the 09:09:51 nudge (s-e919d08c) fired during a
                        // two-minute `secrets-run` the user had just approved.
                        Ok(p) => p || bridge.has_running_gate(&session_id),
...
mod tests {
    ...
    /// F9 (week 35): an APPROVED gate whose command is still executing marks
    /// the session as running one; the poll ORs that into `pending_tray`, so
    /// the two-minute `secrets-run` after its approval no longer draws a
    /// nudge. The mark clears on the guard's drop, whichever way the run ends.
    #[test]
    fn a_running_gate_counts_as_pending_until_its_guard_drops() {
        ...
        let pending = false || bridge.has_running_gate("s1");
        assert_eq!(
            idle_unflagged_decision(OVER, 3, pending, None, false, false, GRACE),
            IdleDecision { chip: false, nudge: false }
        );
        drop(guard);
        assert!(!bridge.has_running_gate("s1"));
        assert!(idle_unflagged_decision(OVER, 3, bridge.has_running_gate("s1"), None, false, false, GRACE).nudge);
    }
