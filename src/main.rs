...
fn run_gui(startup: &mut StartupState) -> Result<()> {
    ...
    let (core, storage_arc, bridge_arc): (Arc<CoreAppState>, Arc<Storage>, Arc<SignalingBridge>) =
        runtime.block_on(async {
            ...
            // This launch's hook secret: the hook subprocesses read it at run
            // time and present it on `/hooks/*`. A failed write leaves the
            // bridge with no token registered, which REFUSES both routes — the
            // hooks then fall back to their no-app wording (a blocked push, a
            // "call action_gate" refusal); never an unauthenticated park.
            match paths.write_hook_token() {
                Ok(token) => bridge.set_hook_token(&token),
                Err(e) => tracing::error!(
                    ?e,
                    "failed to write the hook token — /hooks/* will refuse every call until relaunch"
                ),
            }
            server.set_hook_token_file(paths.hook_token_path.clone());
