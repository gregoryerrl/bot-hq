...
impl PluginManifest {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // `entry` names a file the install path WRITES (`plugin_dir.join(entry)`
        // on a URL install) and the serve path reads; both must stay inside
        // the plugin's own directory. Same test the serve path applies to
        // every requested URL path, so an absolute path, `..`, a backslash, a
        // drive prefix or percent-encoding is refused at validation — before
        // the consent dialog, which shows capabilities, not filenames.
        if !crate::plugins::serve::is_safe_path(&self.entry) {
            return Err(anyhow!(
                "manifest.entry must be a relative path inside the plugin (no '..', '\\\\', \
                 leading '/', drive prefix or percent-encoding; got: {:?})",
                self.entry
            ));
        }
...
mod tests {
    ...
    /// `entry` is a path the install writes (S4, 2026-09-06 sweep): every
    /// shape that escapes the plugin dir is refused at parse time — absolute,
    /// `..`, backslash, a Windows drive prefix (which `Path::join` treats as
    /// absolute and the segment walk alone would pass), percent-encoding —
    /// while ordinary relative entries, nested ones included, still parse.
    #[test]
    fn parse_rejects_an_entry_that_escapes_the_plugin_dir() {
        let manifest = |entry: &str| {
            format!(
                r#"{{"id": "p", "name": "x", "version": "0.1.0", "entry": {}, "requested_capabilities": []}}"#,
        ...
        for bad in [
            ...
            let err = PluginManifest::parse(&manifest(bad)).unwrap_err();
            assert!(
                err.to_string().contains("manifest.entry"),
                "{bad:?} must be refused for its entry, got: {err}"
            );
        }
        for good in ["index.html", "dist/index.html", "ui/v2/panel.html"] {
            let m = PluginManifest::parse(&manifest(good)).unwrap();
            assert_eq!(m.entry, good);
        }
    }
