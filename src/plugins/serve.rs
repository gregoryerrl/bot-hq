...
/// percent-encoding, NULs, absolute paths — including a Windows drive
/// prefix (`C:/…`), which `Path::join` treats as absolute on Windows and
/// which the segment walk alone would pass (`["C:", …]` has no dotted or
/// empty segment). Shared by the serve path and, since 1.0.5, by
/// `PluginManifest::validate` for `entry`: the install path writes
/// `plugin_dir.join(entry)`, so an unchecked entry was an arbitrary file
/// write from a manifest.
...
mod tests {
    ...
    fn refuses_traversal_and_bad_paths() {
        ...
        for bad in [
            ...
            "a%2e%2e/x",
            "a\\..\\x",
            // A drive prefix: absolute on Windows, and invisible to a
            // segment walk (no dotted or empty segment).
            "C:/Windows/x",
            "c:x",
        ] {
            assert_eq!(
