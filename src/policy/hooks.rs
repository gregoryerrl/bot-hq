...
fn classify_push_response(status: reqwest::StatusCode, body: &str) -> PushDecision {
    if !status.is_success() {
        // Carry the app's own text: a 401 names the rebuild fix (a hook binary
...
fn tool_gate_exit(
    ...
    match crate::policy::tool_gate::match_keyword("Bash", command, keywords) {
        Some(GateMode::Gate) => {
            let why = crate::policy::tool_gate::gate_match_detail("Bash", command, keywords);
            (2, Some(gate_refusal_text(command, parked, why.as_ref())))
        }
...
fn gate_refusal_text(
    command: &str,
    parked: Option<&ParkedGate>,
    why: Option<&crate::policy::tool_gate::GateMatch>,
) -> String {
    // F6 (feedback #29): say WHAT matched and WHERE, so a data-string hit —
    // a destructive literal inside a grep pattern — is recognisable at a
    // glance by the agent and by the user reading the card.
    let why_line = why
        .map(|w| format!(" ({}).", w.describe()))
    ...
    let no_dodging = "Do NOT rewrite the command to get around the gated keyword — \
         ...
         don't dodge it.";
    match parked {
        // 0080: an outward publish the reviewer has not read yet is QUEUED, not
        // parked — the agent must neither call `action_gate` nor post the body.
        Some(gate) if gate.queued => {
            let lead = if gate.existing {
                "an identical outward command was ALREADY queued for review"
            } else {
                "bot-hq has QUEUED it for the reviewer"
            };
            format!(
                "QUEUED for review (a Tool Gate stop, NOT an error): `{command}`{why_line}\n\
                 This is an OUTWARD publish and the reviewer has not read its content yet \
                 — {lead} (gate_id: {}). bot-hq posted the body to the channel and \
                 summoned the reviewer; after their turn the gate parks for the user's \
                 approval on its own, unless they file a blocking finding (then it is \
                 withdrawn and a system row says so). Do NOT call `action_gate`, do NOT \
                 post the body yourself, and halting is fine — the queued row keeps the \
                 session from reading as idle. `gate_status(\"{}\")` reports its state.\n\
                 {no_dodging}",
        ...
        Some(gate) => {
            ...
            format!(
                "PARKED for the user's approval (a Tool Gate stop, NOT an error): `{command}`{why_line}\n\
        ...
        None => format!(
            "STOPPED by the bot-hq Tool Gate (normal control flow, NOT a fault): `{command}`{why_line}\n\
...
pub(crate) struct ParkedGate {
    ...
    existing: bool,
    /// 0080: the row is QUEUED for the reviewer, not parked for the user — an
    /// outward publish whose body the reviewer has not read yet.
    queued: bool,
}
...
pub(crate) async fn park_gate(
    ...
    let client = reqwest::Client::builder()
        ...
        .build()
        .ok()?;
    let resp = match with_hook_token(client.post(format!("http://{addr}/hooks/tool-gate")), data_dir)
        .header("content-type", "application/json")
        .body(body)
        .send()
        .await
    ...
    if !status.is_success() {
        // The 401 text names the fix (rebuild a stale hook binary); keep it in
        // the log since the agent only ever sees the call-`action_gate` fallback.
        tracing::warn!(status = status.as_u16(), body = %txt.trim(), "tool-gate auto-park refused");
    }
    classify_park_response(status, &txt)
}

/// Attach this launch's hook secret (`<data_dir>/.local/hook-token`) to a
/// `/hooks/*` request. No file → no header, and the SERVER decides: an app that
/// never wrote one (an older build) enforces nothing; one that did refuses
/// with a 401 whose text names the fix. The client never blocks on the file's
/// absence — a human commit with the app closed must keep working exactly as
/// it did (`check_findings_gate` fails open for the same reason).
fn with_hook_token(req: reqwest::RequestBuilder, data_dir: &Path) -> reqwest::RequestBuilder {
    match crate::paths::read_hook_token(data_dir) {
        Some(token) => req.header(crate::signaling::HOOK_TOKEN_HEADER, token),
        None => req,
    }
}
...
fn classify_park_response(status: reqwest::StatusCode, body: &str) -> Option<ParkedGate> {
    ...
    Some(ParkedGate {
        ...
        queued: v.get("queued").and_then(|q| q.as_bool()).unwrap_or(false),
    })
}
...
fn resolve_hooks_dir(working_repo: &Path) -> Option<PathBuf> {
    // Plain git on purpose: this asks WHERE the hooks live, and the hardened
    // form's `core.hooksPath` override would answer with its own decoy.
    // `rev-parse` runs no config-named program, so nothing is lost.
...
fn render_hook_body(
    ...
    // (backslashes are escapes); double-quote it for spaces. On Unix the path
    // gets the same single-quoting as `--data-dir` — an install under a path
    // with a space (`/Applications/My Tools/…`) or a shell metacharacter
    // otherwise breaks every hook with "not found", and `shell_quote` passes
    // the common metacharacter-free path through byte-identical. The
    // `--data-dir` arg stays single-quoted with its native separators — it's
    // passed literally to bot-hq, which parses Windows paths fine.
    let bin_for_sh = if cfg!(windows) {
        format!("\"{}\"", bot_hq_bin.replace('\\', "/"))
    } else {
        shell_quote(bot_hq_bin)
    };
    let mut cmd = format!(
...
mod tests {
    ...
    /// An install path with a space or a metacharacter is quoted on Unix like
    /// every other argument — unquoted, `exec /Applications/My Tools/bot-hq`
    /// is "exec /Applications/My" and every hook dies with "not found".
    #[cfg(unix)]
    #[test]
    fn render_hook_body_quotes_a_binary_path_with_spaces() {
        let body = render_hook_body(
            HookKind::PrePush,
            "/Applications/My Tools/bot-hq.app/Contents/MacOS/bot-hq",
            Path::new("/home/u/.bot-hq"),
            None,
        );
        assert!(
            body.contains("exec '/Applications/My Tools/bot-hq.app/Contents/MacOS/bot-hq' policy-check pre-push"),
            "body: {body}"
        );
    }
    ...
    fn parked_refusal_stops_the_agent_instead_of_routing_it() {
        ...
        let msg = gate_refusal_text("gh pr create --title x", Some(&gate), None);
        ...
        assert!(
            gate_refusal_text("gh pr create --title x", Some(&existing), None)
        ...
        for parked in [Some(&gate), None] {
            let m = gate_refusal_text("gh pr create --title x", parked, None);
        ...
        assert!(gate_refusal_text("gh pr create --title x", None, None)
            .contains("Call the `action_gate` MCP tool"));
        // F6 (feedback #29): the refusal names the keyword and where it hit,
        // so a data-string match reads as one.
        let why = crate::policy::tool_gate::GateMatch { keyword: "bq rm".into(), col: Some(212) };
        let w = gate_refusal_text("grep -cF \"bq rm -r\" tasks.md", None, Some(&why));
        assert!(w.contains("matched Tool-Gate keyword `bq rm` at col 212"), "got: {w}");
        // 0080: a QUEUED outward publish says so — no action_gate call, no body
        // re-post, halting allowed — and still carries the gate id and the
        // anti-rewording clause.
        let queued = ParkedGate {
            gate_id: "cid-9".into(),
            existing: false,
            queued: true,
        };
        let q = gate_refusal_text("gh issue close 599 --comment x", Some(&queued), None);
        for needle in [
            "QUEUED for review",
            "gate_id: cid-9",
            "Do NOT call `action_gate`",
            "do NOT post the body",
            "halting is fine",
            "Do NOT rewrite the command",
        ] {
            assert!(q.contains(needle), "queued refusal lacks {needle:?}: {q}");
        }
        assert!(!q.contains("PARKED for the user's approval"), "got: {q}");
    }
    ...
    fn classify_park_response_promises_a_gate_only_on_a_real_one() {
        ...
        // 0080: the route says `queued` for an outward publish the reviewer has
        // not read; absent → false, so a pre-0080 app still parks.
        assert_eq!(
            classify_park_response(StatusCode::OK, r#"{"gate_id":"q1","queued":true}"#),
            Some(ParkedGate {
                gate_id: "q1".into(),
                existing: false,
                queued: true
