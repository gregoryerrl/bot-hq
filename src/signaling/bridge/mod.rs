...
mod action_gate;
pub(crate) use action_gate::ParkOutcome;
mod cl_facade;
mod cl_push;
...
/// What an agent's close surfaces once (0081): `card` is the user-facing recap
/// on the close card; `detail` is the executor-facing material behind it (the
/// staleness sweep's hit list, the open advisories) — empty when there is none.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct CloseRecap {
    pub card: String,
    pub detail: String,
}
...
pub struct SignalingBridge {
    ...
    /// Serialises library pushes (F7b, 2026-09-06): two `cl_write_file` calls
    /// in one turn each spawned a push, and the second lost the race on the
    /// remote's ref ("cannot lock ref … expected <old sha>") — harmless when
    /// the first push carried both commits, a lost backup when it did not.
    /// One push at a time; a later push sees everything the earlier ones did.
    library_push_lock: Arc<tokio::sync::Mutex<()>>,
    /// Gated commands currently EXECUTING after an approve (F9, week 35):
    /// `(session_id, choice_id)`. The idle watchdog counts one as pending — a
    ...
    /// one.
    ///
    /// Absent means "this pair has no secret registered" — and since 1.0.5 that
    /// refuses like a wrong secret does; see [`SignalingBridge::mcp_token_matches`]
    /// for why no legitimate caller can be in that state.
    mcp_tokens: std::sync::Mutex<HashMap<String, String>>,
    /// This launch's secret for the hook routes (`/hooks/pre-push`,
    /// `/hooks/tool-gate`), minted by `main.rs` and written 0600 to
    /// `<data_dir>/.local/hook-token` for the hook subprocesses to present.
    /// Those routes carry no per-agent identity, and until 1.0.5 nothing at
    /// all: any local process reaching the port could park an approval card
    /// under an agent's name. `None` (never registered — tests, or a startup
    /// that failed to write the file) REFUSES the routes: a hook that cannot
    /// park falls back to its call-`action_gate` wording, which costs
    /// convenience, never safety.
    hook_token: std::sync::Mutex<Option<String>>,
...
impl SignalingBridge {
    ...
    pub(crate) fn new_with(violations: Option<ViolationsLog>, data_dir: Option<PathBuf>) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            hook_token: std::sync::Mutex::new(None),
    ...
    pub async fn register_session_sequencer(
        ...
        // 0080: a QUEUED outward publish's reviewer summons lived in the ring
        // that just died; the row survived. Re-summon so the queue settles
        // instead of stranding (P1 — the strand path the durable design exists
        // to close).
        self.rearm_queued_outward_for(&session_id).await;
    }
    ...
    /// Mark a gated command as EXECUTING (F9): returns a guard whose drop
    ...
    ///
    /// **Absent registration REFUSES.** Until 1.0.5 a pair with no secret on
    /// file was allowed through, on the argument that it was a session spawned
    /// before secrets existed and refusing it would break a live session at
    /// upgrade time. That argument has no client any more, and the reason is
    /// structural rather than a version count: this server binds an EPHEMERAL
    /// port on every launch and rewrites `signaling-addr`, and a subprocess
    /// carries the server address baked into its mcp-config. A subprocess from
    /// a previous launch — killed or orphaned, it does not matter — therefore
    /// cannot reach THIS server at all; every subprocess that can was spawned
    /// by this process, through the one spawn path that registers a fresh
    /// secret first (`core::session::participant_spawn_config`). What the
    /// allow arm actually admitted was the opposite of its intent: any caller
    /// that invented a session/agent pair — a browser page that found the
    /// port, another local process, a zero-grant plugin iframe — got the
    /// ungated tools, `webview_click` included (2026-09-06 sweep, S1).
    ///
    ...
    /// Register this launch's hook secret (see the `hook_token` field). Called
    /// once from `main.rs` after the token file is written.
    pub fn set_hook_token(&self, token: &str) {
        *self.hook_token.lock().unwrap_or_else(|p| p.into_inner()) = Some(token.to_string());
    }

    /// Does a `/hooks/*` caller hold this launch's hook secret? No secret
    /// registered → false (fail closed — the field doc says why); otherwise a
    /// constant-time compare, same as [`SignalingBridge::mcp_token_matches`].
    pub fn hook_token_matches(&self, presented: Option<&str>) -> bool {
        use subtle::ConstantTimeEq;
        let expected = self
            .hook_token
            .lock()
    ...
    /// The close card's recap (0081): everything the three close nudges used
    /// to surface one refusal at a time — the staleness sweep (demoted to a
    /// report line, F5), the open advisory findings, and whether a CL learnings
    /// delta was written this session — rendered ONCE, on the card the user
    /// reads and in the executor's tool result. Each sub-report still fires at
    /// most once per session (their flags), so a re-parked card after a Reject
    /// carries a shorter recap; the first one is in the channel either way.
    pub async fn close_recap(&self, session_id: &str) -> CloseRecap {
        let mut card: Vec<String> = Vec::new();
        let mut detail: Vec<String> = Vec::new();
        if let Some(sweep) = self.staleness_sweep(session_id).await {
            // Report-only (F5): the card gets one line, the executor gets the
            // hit list it can act on if any hit is real.
            let hits = sweep.lines().filter(|l| l.starts_with("  ")).count();
            card.push(format!(
                "Staleness sweep (advisory — historically vocabulary, not defects): {hits} file \
                 line(s) still use a term this session's CL writes retired; the executor has \
                 the list."
            ));
            detail.push(sweep);
        }
        if let Some(adv) = self.open_advisories_report(session_id).await {
            let n = adv
                .split_whitespace()
                .nth(2)
                ...
                .unwrap_or(0);
            card.push(format!(
                "{n} open advisory finding(s) will archive undispositioned with the session."
            ));
            detail.push(adv);
        }
        let (cl_written, _) = self.close_gate_flags(session_id).await;
        card.push(if cl_written {
            "CL learnings delta: written this session.".to_string()
        } else {
            "CL learnings delta: NOT written this session (the executor was nudged once).".to_string()
        });
        CloseRecap { card: card.join("\n"), detail: detail.join("\n\n") }
    }

    /// Park an AGENT's close for the user's word (0081): a `close` card in the
    /// gate slot — canonical Approve/Reject, the ring latched while it is up.
    /// Approve closes the session through the ordinary `SessionCloseRequest`;
    /// Reject posts a "keep working" row. No in-memory oneshot: a pending row
    /// with no parked oneshot is the post-restart shape `resolve_choice`
    /// already handles, so the card survives a relaunch like any gate.
    /// Returns the card's choice id. An identical pending card is returned
    /// instead of stacking a second one.
    pub async fn park_close_card(
        &self,
        session_id: &str,
        agent: &str,
        recap: &str,
        ...
        if let Some(existing) = storage
            .tray_entries_for_session(session_id)
            .await?
            .into_iter()
            .find(|r| r.status == "pending" && r.kind == crate::storage::QuestionKind::Close.as_str())
        {
        ...
        let _ = self.event_tx.send(SignalingEvent::PendingChoice(PendingChoice {
            ...
            approval: Some(ApprovalContext {
                kind: ViolationKind::GenericApproval,
                action: "close_session".to_string(),
                detail: Some("close-card".to_string()),
                command: None,
            }),
        }));
        Ok(choice_id)
