...
impl SignalingBridge {
    ...
    pub(super) async fn set_session_awaiting(&self, session_id: &str, asker: &str, halt_ring: bool) {
    ...
    pub async fn resolve_choice_confirmable(
        ...
        // withdraw path has always read first; this one now does too. The row's
        // kind and asker ride along (0081): a `close` card's Approve is a close
        // request, decided after the same lift.
        let gate_row = {
            let storage = self.storage.lock().await.clone();
            match storage {
                Some(storage) => match storage.get_tray_entry(choice_id).await {
                    Ok(row) => row
                        ...
                        .map(|row| (row.session_id, row.kind, row.agent)),
                    Err(e) => {
        ...
        if flipped {
            if let Some((session_id, kind, agent)) = gate_row {
                self.notify_ring_gate(&session_id, choice_id, false).await;
                // 0081: a `close` card's verdict, on the flip only (so a duplicate
                // resolve cannot close twice or decline twice). Approve → the same
                // `SessionCloseRequest` the agent's direct close emits, routed by
                // main.rs to `AppState::close_session` — D15's epilogue runs
                // unchanged. Reject → a row telling the executor to keep working;
                // its typed reasoning still arrives as the user's answer row.
                if kind == crate::storage::QuestionKind::Close.as_str() {
                    if matches!(gate_verdict(&picked), crate::policy::ViolationOutcome::Approved) {
                        tracing::info!(session = %session_id, %agent, "close card APPROVED by the user — closing");
                        self.request_session_close(session_id.clone(), agent, false);
                    } else {
                        let storage = self.storage.lock().await.clone();
                        if let Some(storage) = storage {
                            let _ = crate::core::post_system_notice(
                                ...
                                format!(
                                    "[System: the user DECLINED the close ({picked}). The session stays \
                                     open — treat the answer text as the next instruction, and call \
                                     close_session again only when the user says to close.]"
                                ),
                                None,
                            )
                            .await;
...
mod tests {
    ...
    /// 0081: a close card is a gate — it latches the ring when parked and lifts
    /// it on either answer. Reject posts the keep-working row and requests no
    /// close; Approve requests the close (the jsonrpc tests cover Approve).
    #[tokio::test]
    async fn a_rejected_close_card_lifts_the_latch_and_keeps_the_session_open() {
        let bridge = SignalingBridge::new();
        let storage = crate::storage::Storage::memory().await.unwrap();
        storage.create_session("s1", "t", None).await.unwrap();
        bridge.set_storage(storage.clone()).await;
        let (ring_tx, mut ring_rx) = tokio::sync::mpsc::channel(16);
        bridge.register_session_sequencer("s1".into(), ring_tx).await;
        let mut sub = bridge.subscribe();

        let card = bridge.park_close_card("s1", "hands", "recap line").await.unwrap();
        let row = storage.get_tray_entry(&card).await.unwrap().unwrap();
        assert_eq!(row.kind, "close");
        assert_eq!(row.status, "pending");
        assert!(crate::storage::is_gate_row(&row.kind, row.options_json.as_deref()));
        assert!(matches!(
            ring_rx.try_recv(),
            Ok(crate::core::sequencer::SequencerCommand::GateOpened { choice_id }) if choice_id == card
        ));
        // Parking twice returns the same card.
        assert_eq!(bridge.park_close_card("s1", "hands", "again").await.unwrap(), card);

        bridge
            .resolve_choice(&card, "Reject — one more thing first".into())
            .await
            .unwrap();
        assert_eq!(storage.get_tray_entry(&card).await.unwrap().unwrap().status, "answered");
        assert!(matches!(
            ring_rx.try_recv(),
            Ok(crate::core::sequencer::SequencerCommand::GateResolved { choice_id }) if choice_id == card
        ));
        ...
        assert_eq!(close_requests, 0, "Reject must not close");
        let rows = storage.recent_row_bodies_upto("s1", i64::MAX, 20).await.unwrap();
        assert!(
            rows.iter().any(|r| r.contains("DECLINED the close")),
            "the executor is told to keep working: {rows:?}"
        );
    ...
    async fn ask_user_choice_auto_supersedes_reask_same_prompt() {
        ...
        // `ask_user_choice` is non-blocking: it returns the parked ack once
        // the row is inserted, so awaiting it IS the synchronization. (This
        // used to spawn + sleep 20 ms, which lost the race on Windows CI.)
        let first = bridge
            .ask_user_choice(
                "s1".into(),
                "hands".into(),
            ...
            .await
            .unwrap();
        assert!(first.contains("parked"), "ack: {first}");
        ...
        let second = bridge
            ...
            .await
            .unwrap();
        assert!(second.contains("parked"), "ack: {second}");
    ...
    async fn distinct_prompts_accumulate_not_superseded() {
        ...
        // Non-blocking calls: each returns its parked ack after the insert,
        // so the two awaits are the synchronization (no spawn + sleep).
        bridge
            .ask_user_choice(
                "s1".into(),
                "hands".into(),
    ...
    async fn supersede_question_links_old_to_new() {
        ...
        // supersede_question_with_new is non-blocking like ask_user_choice:
        // it returns a parked ack, not the pick (which arrives out-of-band) —
        // so awaiting it directly is the synchronization (no spawn + sleep).
        let ack = bridge
            .supersede_question_with_new(
                "s1".into(),
                "hands".into(),
