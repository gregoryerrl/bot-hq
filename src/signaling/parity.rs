...
///
/// `cl_edit_file` joined on 2026-09-06 (week-35 Batch 5, feedback #30): the
/// in-place CL correction is gated on `WriteContextLibrary` exactly like
/// `cl_write_file`, whose pre-rc3 name-gate line (`CL_MUTATE` above) refused
/// EYES. The name gate never knew `cl_edit_file`, so the comparison is
/// definitionally inapplicable again. The gating is the USER's call, asked in
/// session s-107aa99d as tray df02d2a8 (gate it like `cl_write_file` /
/// ungated for every role / skip the tool) and answered 2026-09-06 with the
/// first option, verbatim: *"Gate it like cl_write_file (write_context_library
/// — EYES cannot edit CL content; adds a SANCTIONED_DIVERGENCE entry citing
/// this answer) — recommended"*. Entry and decision in one commit, the same
/// procedure as `flag_finding`.
const SANCTIONED_DIVERGENCE: &[&str] = &["flag_finding", "close_session", "cl_edit_file"];
