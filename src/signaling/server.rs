...
/// The header the hook subprocesses present on `/hooks/*` — this launch's
/// secret from `<data_dir>/.local/hook-token`. Shared with the client side in
/// `policy::hooks` so the two cannot drift.
pub const HOOK_TOKEN_HEADER: &str = "x-bot-hq-hook-token";

/// Body of the 401 on `/hooks/*`. The common non-attack cause is a hook binary
/// older than the running app: a hook body bakes in the ABSOLUTE path of
/// whichever `bot-hq` installed it (`policy::hooks` uses `current_exe()` —
/// `target/release/bot-hq` on a source-run machine, the app bundle's binary
/// on an install), so a stale build at that path sends no header. The text
/// therefore points at that path — a bare "run `cargo build`" would rebuild
/// the debug binary and change nothing — rather than reading as a security
/// alert.
pub const HOOK_TOKEN_REFUSAL: &str = "unauthorized: missing or wrong X-Bot-Hq-Hook-Token. \
    If you just rebuilt or upgraded bot-hq, the hook binary predates this launch's hook token: \
    the hook runs the bot-hq path written inside .git/hooks/pre-push (or .bot-hq sidecar) — \
    rebuild THAT binary, or relaunch bot-hq and start a session (hooks are reinstalled at spawn, \
    pointing at the running binary). Otherwise something other than a bot-hq hook is calling \
    this route.";
...
pub struct SignalingServer {
    ...
    /// Path of the persisted `hook-token` file (see [`HOOK_TOKEN_HEADER`]);
    /// removed on clean shutdown like `addr_file`, and for the same reason.
    hook_token_file: Option<PathBuf>,
}
...
impl SignalingServer {
    ...
    /// Same for the `hook-token` file, written right after the address.
    pub fn set_hook_token_file(&mut self, path: PathBuf) {
        self.hook_token_file = Some(path);
    }
}
...
impl Drop for SignalingServer {
    fn drop(&mut self) {
        ...
        if let Some(path) = self.hook_token_file.take() {
            let _ = std::fs::remove_file(path);
        }
    }
}
...
async fn handle_request(
    ...
    // Nothing here is for a browser. A page that found the port sends a
    // cross-origin POST with `Origin` (and `Sec-Fetch-*`) it cannot suppress;
    // none of bot-hq's clients — the claude-code MCP client, the hook
    // subprocesses — send either. Refused before any route, so a browser can
    // neither reach a tool nor park a card (2026-09-06 sweep, S1). See
    // `core::loopback` for why the header test is sufficient.
    if crate::core::loopback::browser_originated(req.headers()) {
        warn!(%path, "request refused: browser-originated (Origin / Sec-Fetch-* present)");
        return Ok(text_response(StatusCode::FORBIDDEN, "not for browsers"));
    }

    // Dedicated routes for the hook subprocesses. They are NOT agents and have
    // no per-(session,agent) MCP identity, so they bypass the JSON-RPC +
    // capability gate — which until 1.0.5 meant they had no identity at all:
    // any local process that reached the port could park an approval card in
    // the user's tray under an agent's name, and an Approve would run it. Both
    // routes now require this launch's hook secret (minted by `main.rs`,
    // written 0600 to `<data_dir>/.local/hook-token`, read by the hook binary
    // at run time), checked BEFORE the body is read.
    if path == "/hooks/pre-push" || path == "/hooks/tool-gate" {
        let presented = req
            .headers()
            .get(HOOK_TOKEN_HEADER)
            .and_then(|v| v.to_str().ok());
        if !bridge.hook_token_matches(presented) {
            warn!(%path, presented = presented.is_some(), "hook route refused: missing or wrong hook token");
            return Ok(text_response(StatusCode::UNAUTHORIZED, HOOK_TOKEN_REFUSAL));
        }
        // The git pre-push hook (push_gate=ask) calls `request_approval`
        // directly and blocks until the user picks.
        if path == "/hooks/pre-push" {
            return Ok(handle_pre_push(req.into_body(), bridge).await);
        }
        // The PreToolUse Tool Gate hook parks the approval for the command it
        // just blocked, so the agent doesn't have to convert the refusal into
        // an `action_gate` call by hand (issues.md #29).
    ...
    // Observation only, this release: the MCP client is expected to send
    // `application/json`, and once the log shows it always does, this becomes
    // a refusal (a second layer under the browser guard above — a browser
    // cannot send that content type without a preflight). Not enforced yet
    // because a wrong assumption here would cut every tool off at once.
    let content_type_ok = req
        ...
        .unwrap_or(false);
    if !content_type_ok {
        warn!(%path, "MCP request without an application/json content-type (observation only; not refused)");
    }
    ...
    // that agent's own mcp-config, so possessing it IS being that agent — and
    // since 1.0.5 a pair with no secret registered is refused too, so an
    // invented pair gets nothing (`mcp_token_matches` says why that is safe).
...
async fn handle_tool_gate(body: Incoming, bridge: Arc<SignalingBridge>) -> Response<Full<Bytes>> {
    ...
    match bridge.park_gated_command(session_id, agent, command).await {
        Ok(crate::signaling::bridge::ParkOutcome::Parked { gate_id, existing, note }) => {
            text_response(
                StatusCode::OK,
                &json!({ "gate_id": gate_id, "existing": existing, "note": note }).to_string(),
            )
        }
        // 0080: the hook's refusal text must say QUEUED, not parked — the
        // agent must neither call `action_gate` again nor post the body.
        Ok(crate::signaling::bridge::ParkOutcome::Queued { gate_id, existing }) => {
            text_response(
                StatusCode::OK,
                &json!({ "gate_id": gate_id, "existing": existing, "queued": true }).to_string(),
            )
        }
        Err(e) => {
...
mod tests {
    ...
    /// The hook secret the hook-route tests register and present.
    const HOOK_SECRET: &str = "hook-secret-for-tests";
    ...
    async fn pre_push_route_approve_then_reject() {
        ...
        bridge.set_hook_token(HOOK_SECRET);
        ...
        let call = tokio::spawn(async move {
            ...
            let resp = client_a
                .post(&url_a)
                .header(HOOK_TOKEN_HEADER, HOOK_SECRET)
    ...
    /// **A local process cannot park a card** (1.0.5, sweep S2). Both hook
    /// routes refuse a caller without this launch's hook secret BEFORE reading
    /// the body — no header, a wrong header, and a bridge that never registered
    /// one (the failed-startup-write case) all 401 — and the refusal text
    /// names the rebuild fix, because a stale hook binary is the common
    /// non-attack cause. The MCP route is untouched by the hook secret.
    #[tokio::test]
    async fn hook_routes_refuse_without_this_launch_secret() {
        use crate::policy::ViolationsLog;
        let data = tempfile::tempdir().unwrap();
        let bridge =
            SignalingBridge::with_policy(ViolationsLog::new(data.path()), data.path().to_path_buf());
        ...
        let server = start_signaling_server(Arc::clone(&bridge)).await.unwrap();
        let client = reqwest::Client::new();
        let body = json!({ "session_id": "s1", "agent": "hands", "command": "gh pr create -t x", "branch": "x" })
            .to_string();
        let post = |route: &'static str, token: Option<&'static str>| {
            let (client, body) = (client.clone(), body.clone());
            let url = format!("http://{}{}", server.local_addr, route);
            async move {
                let mut req = client.post(&url).header("content-type", "application/json");
                if let Some(t) = token {
                    req = req.header(HOOK_TOKEN_HEADER, t);
                }
        ...
        // No secret registered yet: even the right-looking header is refused.
        let (status, txt) = post("/hooks/tool-gate", Some(HOOK_SECRET)).await;
        assert_eq!(status, StatusCode::UNAUTHORIZED, "unregistered bridge must refuse");
        assert!(
            txt.contains(".git/hooks/pre-push"),
            "refusal points at the baked hook-binary path, not a bare `cargo build`: {txt}"
        );

        bridge.set_hook_token(HOOK_SECRET);
        for route in ["/hooks/tool-gate", "/hooks/pre-push"] {
            let (status, txt) = post(route, None).await;
            assert_eq!(status, StatusCode::UNAUTHORIZED, "{route}: no header");
            assert_eq!(txt, HOOK_TOKEN_REFUSAL);
            let (status, _) = post(route, Some("not-the-secret")).await;
            assert_eq!(status, StatusCode::UNAUTHORIZED, "{route}: wrong header");
        }
        // The right secret parks (tool-gate answers at once; pre-push would
        // block on the user, so only the non-blocking route is exercised here).
        let (status, txt) = post("/hooks/tool-gate", Some(HOOK_SECRET)).await;
        assert_eq!(status, StatusCode::OK, "{txt}");
        assert!(txt.contains("gate_id"));
        // Nothing was parked by the refused calls: exactly one tray row exists.
        let rows = storage.tray_entries_for_session("s1").await.unwrap();
        assert_eq!(rows.len(), 1, "refused calls must not park: {rows:?}");
    }

    /// The WIRE, client side: the real hook client (`policy::hooks::park_gate`)
    /// reads the secret off disk and presents it. With the token file present
    /// the park succeeds; delete the file and the same call is refused — so
    /// removing the header attach in the client fails this test, not just the
    /// server-side one above.
    #[tokio::test]
    async fn hook_client_presents_the_secret_it_reads_from_disk() {
        use crate::policy::ViolationsLog;
        let data = tempfile::tempdir().unwrap();
        let paths = crate::paths::Paths::for_data_dir(data.path().to_path_buf());
        paths.init().unwrap();
        let bridge =
            SignalingBridge::with_policy(ViolationsLog::new(data.path()), data.path().to_path_buf());
        let storage = crate::storage::Storage::memory().await.unwrap();
        ...
        bridge.set_storage(storage).await;
        let server = start_signaling_server(Arc::clone(&bridge)).await.unwrap();
        paths.write_signaling_addr(server.local_addr).unwrap();
        let token = paths.write_hook_token().unwrap();
        bridge.set_hook_token(&token);

        let parked = crate::policy::hooks::park_gate(data.path(), "s1", "hands", "gh pr create -t x").await;
        assert!(parked.is_some(), "client with the token file parks");

        std::fs::remove_file(&paths.hook_token_path).unwrap();
        let parked = crate::policy::hooks::park_gate(data.path(), "s1", "hands", "gh pr create -t y").await;
        assert!(parked.is_none(), "client without the token file is refused (401 → None)");
    }
    ...
    ///
    /// The cases that matter are all here: the right secret works, the WRONG
    /// secret is refused, dropping the secret is refused — and, since 1.0.5, a
    /// pair with NO secret registered is refused too. That last case used to
    /// be the opposite ("keeps a pre-upgrade session alive"), which admitted
    /// any invented pair; no legitimate caller can be in that state because a
    /// subprocess from an earlier launch cannot reach this launch's ephemeral
    /// port (`mcp_token_matches`).
    #[tokio::test]
    async fn an_mcp_call_needs_the_secret_that_belongs_to_the_agent() {
        ...
        assert_eq!(
            ...
            .await,
            403,
            "an UNREGISTERED pair is refused — an invented session/agent must get nothing"
        );
        assert_eq!(
            post(
                format!("http://{}/sessions/nope/nobody/any-token/mcp", server.local_addr),
                body
            )
            .await,
            403,
            "an invented pair with an invented token is refused"
        );
    }

    /// A browser page that found the port gets nothing from any route: the
    /// request carries `Origin` (a cross-origin POST always does) or
    /// `Sec-Fetch-*`, and both are refused before the token is even looked
    /// at — with a valid token, on the MCP route and on a hook route alike.
    #[tokio::test]
    async fn browser_originated_requests_are_refused_on_every_route() {
        let bridge = SignalingBridge::new();
        bridge.register_mcp_token("s1", "hands", "hands-secret");
        bridge.set_hook_token(HOOK_SECRET);
        let server = start_signaling_server(Arc::clone(&bridge)).await.unwrap();
        let client = reqwest::Client::new();
        let mcp = format!("http://{}/sessions/s1/hands/hands-secret/mcp", server.local_addr);
        let hook = format!("http://{}/hooks/tool-gate", server.local_addr);
        for (url, extra) in [(mcp.clone(), None), (hook.clone(), Some(HOOK_SECRET))] {
            for (k, v) in [("origin", "http://evil.example"), ("origin", "null"), ("sec-fetch-mode", "no-cors")] {
                let mut req = client
                    .post(&url)
                    .header("content-type", "application/json")
                    .header(k, v)
                ...
                assert_eq!(resp.status(), StatusCode::FORBIDDEN, "{url} with {k}: {v}");
            }
        }
        // The same request without the browser headers is served.
        let resp = client
            .post(&mcp)
            .header("content-type", "application/json")
            .body(json!({"jsonrpc": "2.0", "id": 1, "method": "tools/list"}).to_string())
