...
mod tests {
    ...
    async fn filed_feedback_is_readable_across_sessions() {
        // The whole point: filed from another project's session, read with no
        // session scope at all by a later bot-hq session.
        let s = db().await;
        s.create_session("s-other", "t", None).await.unwrap();
        s.insert_feedback(
            "s-other",
