...
/// [`repair_migration_checksums`] brings the databases the CRLF builds
/// stamped forward to it (and the ones stamped by a superseded text — see
/// [`SUPERSEDED_MIGRATION_CHECKSUMS`]). The codebase already knew `include_str!` is
...
/// Digests that applied rows of a migration may still hold from a build that
/// embedded an EARLIER TEXT of that migration — `(version, sha384 hex)`, the
/// LF and the CRLF form of each superseded text. A row holding one of these
/// is brought forward to the current digest exactly like a CRLF-stamped row,
/// because the migration's schema effect is unchanged: only a comment moved.
///
/// The superseded texts themselves are deliberately NOT in this repository
/// (the comments were reworded on 2026-09-09 to drop names that had no
/// business in a public tree), so these digests cannot be recomputed here.
/// Each was captured from two independent sources before the edit — the
/// file's own `sha384` and the `_sqlx_migrations` row of a database that had
/// applied it — and the pre-edit blobs remain in the maintainer's private
/// archive of the repository under these git blob ids:
/// `0039_agent_feedback.sql` = `972a65371b0d3734eeecf59854522e6089f87f7a`,
/// `0080_tray_queued_body_row.sql` = `db011d6896bb1e1cc5e54b09b156123a251852ee`.
const SUPERSEDED_MIGRATION_CHECKSUMS: &[(i64, &str)] = &[
    // 0039, text through 2026-09-09: LF, then CRLF.
    (39, "3ab88439259257927ea60f31367257551206fdc8a7f89b21f48f083d4b2c3f809bc7e17793eef1ce6759fe52a8fba7a8"),
    (39, "f9f0b136e13dc2519175a90711cddb82e60acc262ccea98f86aff6f46f4ba09bab4d27faaf2dc260cc44885afefe3f48"),
    // 0080, text through 2026-09-09: LF, then CRLF.
    (80, "05fa0b8887e36229307a95149fc8db54a7e99b1d8afd0dbcfa37687045fa34c75129a57d7bdd26d479f8bb0d85b2544d"),
    (80, "2d584c1b1704c663658b6669efcab2b20a2e64bce3b33f5ac9b1a22b7c4a7e0a2322dff59670bf42a6701c579dae99ef"),
];

/// The 48 bytes of a sha384 written as 96 hex digits; `None` for anything
/// else, so a typo in [`SUPERSEDED_MIGRATION_CHECKSUMS`] is a failing test
/// rather than a digest that silently matches nothing.
fn decode_sha384_hex(hex: &str) -> Option<[u8; 48]> {
    if hex.len() != 96 {
        return None;
    }
    let mut out = [0u8; 48];
...
/// The superseded digests recorded for `version`, decoded.
fn superseded_checksums(version: i64) -> impl Iterator<Item = [u8; 48]> {
    SUPERSEDED_MIGRATION_CHECKSUMS
        .iter()
        .filter(move |(v, _)| *v == version)
        .filter_map(|(_, hex)| decode_sha384_hex(hex))
}

/// Rewrite the `_sqlx_migrations` checksums a CRLF-checkout build stamped —
/// or a build that embedded a superseded text of the migration — to the LF
/// digests [`MIGRATOR`] carries, so the migrator's mismatch check passes for
/// a migration whose schema effect has not changed. Returns how many rows it
/// rewrote.
///
...
/// what it holds is the sha384 of the CRLF form of the same text, or one of
/// the digests listed for that version in [`SUPERSEDED_MIGRATION_CHECKSUMS`].
/// Anything else is left exactly as found, so the migrator still refuses a
/// migration that was genuinely edited after it was applied. A database with
/// no `_sqlx_migrations` table yet (a fresh install) is left for the migrator
/// to create; this never creates the table itself. All rewrites land in one
...
pub(crate) async fn repair_migration_checksums(pool: &SqlitePool) -> Result<usize> {
    ...
    for migration in MIGRATOR.iter() {
        ...
        let known_prior = stored == crlf_checksum(&migration.sql).as_slice()
            || superseded_checksums(migration.version).any(|d| d.as_slice() == stored);
        if !known_prior {
            continue;
        }
...
impl Storage {
    ...
    pub async fn open(db_path: &Path) -> Result<Self> {
        ...
        // Checksums stamped by a CRLF-checkout build or by a superseded
        // migration text, brought forward BEFORE the migrator compares them —
        // see `MIGRATOR` for the 1.0.1 story and
        // `SUPERSEDED_MIGRATION_CHECKSUMS` for the reworded comments.
        match repair_migration_checksums(&pool)
            .await
            .context("repairing migration checksums stored by an earlier build")?
        {
            0 => {}
            n => tracing::info!(
                rows = n,
                "repaired migration checksums: they were stored by a build that embedded the \
                 migrations with CRLF line endings (bot-hq 1.0.0 or earlier on Windows) or with \
                 an earlier wording of a comment; the stored digests were rewritten to this \
                 build's"
            ),
        }
        MIGRATOR
...
mod tests {
    ...
    async fn migration_checksum_repair_recovers_a_db_stamped_by_a_crlf_build() {
        ...
        stamp_crlf(s.pool()).await;
        let repaired = repair_migration_checksums(s.pool()).await.unwrap();
        assert_eq!(repaired, MIGRATOR.iter().count());
        assert_eq!(stamps(s.pool()).await, embedded_stamps());
        assert_eq!(repair_migration_checksums(s.pool()).await.unwrap(), 0);
        s.pool().close().await;
    }
    ...
    async fn migration_checksum_repair_does_not_create_the_table_on_a_fresh_db() {
        let pool = SqlitePoolOptions::new()
            ...
            .await
            .unwrap();
        assert_eq!(repair_migration_checksums(&pool).await.unwrap(), 0);
    ...
    /// (f) The superseded-text table is well-formed and says something: every
    /// entry decodes to 48 bytes, names a migration this build embeds, and
    /// differs from both digests the build can derive for it (LF and CRLF of
    /// the CURRENT text) — an entry equal to either would be dead weight, and
    /// a typo would be a digest that matches nothing and rescues nobody.
    #[test]
    fn superseded_migration_checksums_are_well_formed_and_not_current() {
        assert!(!SUPERSEDED_MIGRATION_CHECKSUMS.is_empty());
        for (version, hex) in SUPERSEDED_MIGRATION_CHECKSUMS {
            let digest = decode_sha384_hex(hex)
                .unwrap_or_else(|| panic!("migration {version}: {hex:?} is not 96 hex digits"));
            let current = MIGRATOR
                .iter()
                .find(|m| m.version == *version)
                .unwrap_or_else(|| panic!("migration {version} is not embedded in this build"));
            assert_ne!(digest.as_slice(), &*current.checksum, "migration {version}: entry is the LF digest");
            assert_ne!(
                digest.to_vec(),
                crlf_checksum(&current.sql),
                "migration {version}: entry is the CRLF digest"
            );
        }
        assert_eq!(decode_sha384_hex("ab"), None);
        assert_eq!(decode_sha384_hex(&"zz".repeat(48)), None);
    }

    /// (g) A database whose rows still hold a superseded digest — stamped by
    /// a build that embedded the earlier wording of the comment, in either
    /// line-ending form — opens under this build, exactly like (b): the
    /// second `Storage::open` is the upgrade launch, and it must succeed and
    /// leave the current digests behind. Each entry is exercised on its own
    /// so one stale row cannot hide behind another.
    #[tokio::test]
    async fn migration_checksum_repair_recovers_a_db_stamped_by_a_superseded_text() {
        for (version, hex) in SUPERSEDED_MIGRATION_CHECKSUMS {
            let dir = tempfile::tempdir().unwrap();
            let path = dir.path().join("bot-hq.db");
            let s = Storage::open(&path).await.unwrap();
            let superseded = decode_sha384_hex(hex).unwrap().to_vec();
            sqlx::query("UPDATE _sqlx_migrations SET checksum = ? WHERE version = ?")
                .bind(&superseded)
                .bind(version)
                .execute(s.pool())
            ...
            let s = Storage::open(&path)
                .await
                .unwrap_or_else(|e| panic!("migration {version} stamped {hex}: must open, got {e:#}"));
            assert_eq!(
                stamps(s.pool()).await,
                embedded_stamps(),
                "migration {version}: the row must now hold the current digest"
            );
            assert_eq!(repair_migration_checksums(s.pool()).await.unwrap(), 0);
            s.pool().close().await;
        }
