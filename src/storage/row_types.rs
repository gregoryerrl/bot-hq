...
pub struct Session {
    ...
    /// When the session was last REOPENED from closed (0081), RFC3339-Z; NULL
    /// until the first reopen. The bound an agent's close approval must
    /// postdate: an Approve given before a reopen is void, as the REOPENED
    /// notice has always said and two agents ignored.
    #[serde(default)]
    #[sqlx(default)]
...
pub struct AgentConfig {
    ...
    /// claude-code settings to merge into `--settings` at spawn, copied from
    /// the chosen [`Model`]. `agent_configs` has no such column, so a row read
    /// from that table always carries `None` — the per-agent fallback path
    /// predates the registry and gets no injection.
    #[serde(default)]
    #[sqlx(default)]
...
pub struct Model {
    ...
    ///
    /// Read by one thing: the pump compares it with the window the CLI
    /// reports and posts a notice when they disagree (a model the CLI's
    /// catalog does not know silently gets 200k). Never a meter denominator.
    #[serde(default)]
    pub context_window: Option<i64>,
    /// claude-code settings merged into the `--settings` JSON at spawn — a JSON
    /// object, or `None`. First use: `modelOverrides`, so a model id newer
    /// than the installed CLI's catalog keeps its real window (0079).
    #[serde(default)]
    #[sqlx(default)]
    pub cli_settings: Option<String>,
}
...
pub enum QuestionKind {
    ...
    Request,
    /// An AGENT's close of the session, parked for the user's word (0081): a
    /// GATE like [`Self::Approval`] — canonical `["Approve","Reject"]`, the
    /// gate slot, latches the ring — whose Approve closes the session instead
    /// of running a command. Its own kind so the resolve path can tell it from
    /// a command gate durably (a `command_text` sentinel would be executed as a
    /// shell command by the post-restart path) and so `close_approved_since_open`
    /// can find it by name.
    Close,
...
pub struct SessionTrayEntry {
    ...
    /// For a QUEUED outward publish (0080): the `messages.id` of the body row
    /// bot-hq posted for the reviewer. Settlement flips the row to `pending`
    /// once the reviewer's cursor is at or past it. NULL on every other row.
    #[serde(default)]
    #[sqlx(default)]
