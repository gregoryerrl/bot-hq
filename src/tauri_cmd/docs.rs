...
fn untracked_diff(repo: &std::path::Path) -> String {
    ...
    let listing = match crate::core::git::hardened(repo)
...
pub(crate) async fn compute_apply_diff_inner(
    ...
    let result = tokio::task::spawn_blocking(
        move || -> std::io::Result<(String, Option<String>)> {
            let mut cmd = crate::core::git::hardened(&repo);
            cmd.arg("diff").arg("--no-color").args(crate::core::git::DIFF_HARDENING_ARGS);
