...
async fn seeded_bridge() -> Arc<SignalingBridge> {
    ...
    bridge.register_mcp_token("s1", "hands", TOKEN);
...
/// The per-agent secret every test registers for (s1, hands): since 1.0.5 a
/// pair with no secret registered is refused, so the token-less URL shape is
/// a 403 everywhere below except where that refusal is the point.
const TOKEN: &str = "hands-test-secret";
const MCP_PATH: &str = "/sessions/s1/hands/hands-test-secret/mcp";
...
async fn server_parse_error_returns_jsonrpc_envelope() {
    ...
    let addr = server.local_addr;
    let (status, body) =
        http_post_with_status(addr, MCP_PATH, "{ not valid json").await;
